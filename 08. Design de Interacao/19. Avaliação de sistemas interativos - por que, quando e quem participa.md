...
## Avaliar significa produzir evidências sobre a qualidade da interação
...
## Toda avaliação começa por objetivo, momento e participantes
...
```mermaid
flowchart TD
    Avaliacao["Planejamento da<br>avaliação em IHC"] --> Porque["1. Por que avaliar?<br>(Objetivos e justificativa)"]
...
## Avaliação formativa e somativa têm finalidades diferentes

A literatura de IHC divide o processo de avaliação em duas grandes categorias temporais e funcionais:

```mermaid
flowchart TD
    subgraph Formativa ["Avaliação Formativa (Durante a construção)"]
        F1["Foco: Identificar problemas<br>e orientar correções"]
        F2["Artefatos: Sketches, wireframes<br>e protótipos"]
    end
    
    subgraph Somativa ["Avaliação Somativa (Produto final)"]
        S1["Foco: Medir qualidade final<br>e atestar metas"]
        S2["Artefatos: Software pronto<br>e instalado"]
    end
...
## Inspeção, observação e investigação produzem evidências diferentes
...
```mermaid
flowchart TD
    Familias["Métodos de avaliação<br>de sistemas interativos"] --> Inspecao["1. Avaliação por Inspeção<br>(Especialistas examinam a interface)"]
...
## Comparação entre famílias de métodos
...
## Avaliação também pode comparar tempo, unidades e níveis
...
```mermaid
flowchart TD
    Dimensoes["Dimensões de<br>comparação na avaliação"] --> Long["1. Avaliação Longitudinal<br>(Eixo temporal /<br>Através do tempo)"]
    Dimensoes --> Lat["2. Avaliação Latitudinal<br>(Eixo horizontal<br>/ Unidades equivalentes)"]
    Dimensoes --> Alt["3. Avaliação Altitudinal<br>(Eixo vertical<br>/ Níveis hierárquicos)"]
```
...
### Avaliação latitudinal ou transversal
...
## Método, momento e participantes devem responder à pergunta de avaliação
...
- **Próximo artigo:** [[20. Avaliação por inspeção - avaliação heurística e percurso cognitivo]]
