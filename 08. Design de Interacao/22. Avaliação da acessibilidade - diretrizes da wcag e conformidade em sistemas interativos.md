...
```mermaid
graph TD
    W3C["W3C (World Wide Web Consortium)"] --> WAI["WAI (Web Accessibility Initiative)"]
    WAI --> WCAG["WCAG (Web Content Accessibility Guidelines)"]
    WCAG --> Avaliacao["Avaliação da conformidade em IHC"]
```
...
```mermaid
graph TD
    WCAG["Os 4 Princípios da WCAG (POUR)"] --> P1["1. Percebível (Perceivable)<br>(Informação visível ou audível aos sentidos)"]
    WCAG --> P2["2. Operável (Operable)<br>(Interface navegável por diferentes meios)"]
...
```mermaid
graph TD
    Metodos["Métodos de avaliação da acessibilidade"] --> Aut["1. Avaliação Automatizada<br>(Software validador de código)"]
    Metodos --> Insp["2. Inspeção Manual por Teclado e Código<br>(Verificação semântica e visual)"]
    Metodos --> Emp["3. Teste Empírico com Tecnologias Assistivas<br>(Leitores de tela e usuários com deficiência)"]
```
