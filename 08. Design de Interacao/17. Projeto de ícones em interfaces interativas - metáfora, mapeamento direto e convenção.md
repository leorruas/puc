...
```mermaid
flowchart TD
    Projeto["Estratégias de<br>projeto de ícones"] --> Metafora["1. Metáfora<br>(Transferência de outro domínio)"]
    Projeto --> Mapeamento["2. Mapeamento direto<br>(Semelhança física<br>com a função)"]
    Projeto --> Convenção["3. Convenção<br>(Padrão universal aprendido)"]
```
...
```mermaid
flowchart TD
    Diretrizes["Diretrizes de<br>usabilidade para ícones"] --> D1["1. Acompanhar de<br>rótulos de texto<br>(Icon + Label)"]
    Diretrizes --> D2["2. Manter consistência familiar<br>no Design System"]
    Diretrizes --> D3["3. Fornecer<br>Dicas de Ferramenta<br>(Tooltips em desktop)"]
    Diretrizes --> D4["4. Garantir área<br>de clique/toque adequada<br>(44x44px em mobile)"]
    Diretrizes --> D5["5. Garantir alto contraste<br>e distinção clara de estados"]
```
