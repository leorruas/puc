...
```mermaid
graph TD
    Gestalt["Princípios gestálticos de percepção visual"] --> P1["1. Proximidade"]
    Gestalt --> P2["2. Boa continuidade (Alinhamento)"]
...
```mermaid
graph TD
    Regras["As Oito Regras de Ouro de Shneiderman"] --> R1["1. Perseguir a consistência"]
    Regras --> R2["2. Fornecer atalhos"]
    Regras --> R3["3. Fornecer feedbacks informativos"]
    Regras --> R4["4. Marcar o final dos diálogos"]
    Regras --> R5["5. Fornecer prevenção de erros<br>e forma simples de corrigi-los"]
    Regras --> R6["6. Permitir a reversão de ações"]
    Regras --> R7["7. Fornecer controle de iniciativa do usuário"]
    Regras --> R8["8. Reduzir a carga de memória de trabalho"]
```
