...
```mermaid
graph TD
    A["Usuários específicos (Quem)"] --- D["Usabilidade emergente (Eficácia, Eficiência, Satisfação)"]
    B["Objetivos específicos (O quê)"] --- D
    C["Contexto de uso específico (Onde/Como)"] --- D
```
...
```mermaid
graph TD
    Pesquisa["Pesquisa empírica<br>(Questionários / Logs / Censos)"] --> Sintese["Agregação estatística<br>de dados"]
...
```mermaid
graph TD
    Sub1["Pesquisa qualitativa e quantitativa"] --> Sub2["Identificação de padrões comportamentais"]
    Sub2 --> Sub3["Criação de arquétipos humanos (Personas)"]
    Sub3 --> Sub4["Decisões de design guiadas por objetivos reais"]
```
...
graph TD
    subgraph Sensorial ["Experiência sensorial e social"]
        ...
        Ve["1. O que vê?"]
        Ouve["2. O que ouve?"]
        PensaSente["3. O que pensa e sente?"]
        FalaFaz["4. O que fala e faz?"]
    end
    subgraph Emocional ["Balanço emocional"]
        direction LR
        Dores["5. Dores (Frustrações e medos)"]
        Ganhos["6. Ganhos (Desejos e necessidades)"]
    end
```
