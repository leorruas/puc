...
```mermaid
graph TD
    Inspecao["Métodos de avaliação por inspeção"] --> AH["1. Avaliação Heurística<br>(Guiada pelas 10 Heurísticas de Nielsen)"]
    Inspecao --> PC["2. Percurso Cognitivo<br>(Guiado por tarefas e 4 perguntas cognitivas)"]
```
...
```mermaid
graph TD
    Nielsen["As 10 Heurísticas de Usabilidade de Nielsen"] --> H1["1. Visibilidade do estado do sistema"]
    Nielsen --> H2["2. Equivalência entre o sistema e o mundo real"]
    Nielsen --> H3["3. Controle e liberdade do usuário"]
    Nielsen --> H4["4. Consistência e padrões"]
    Nielsen --> H5["5. Prevenção de erros"]
    Nielsen --> H6["6. Reconhecer em vez de recordar"]
    Nielsen --> H7["7. Flexibilidade e eficiência de uso"]
    Nielsen --> H8["8. Estética e design minimalista"]
    Nielsen --> H9["9. Diagnóstico e recuperação de erros"]
    Nielsen --> H10["10. Ajuda e documentação"]
```
...
```mermaid
graph TD
    P1["1. Preparação<br>(Todos os avaliadores)"] --> P2["2. Coleta de dados e interpretação<br>(Cada avaliador individualmente)"]
    P2 --> P3["3. Consolidação e relato dos resultados<br>(Todos os avaliadores juntos)"]
```
...
```mermaid
graph TD
    PC["Percurso Cognitivo por etapa de tarefa"] --> Q1["1. O usuário tentará alcançar o efeito correto?"]
    PC --> Q2["2. O usuário perceberá que a ação correta está disponível?"]
    PC --> Q3["3. O usuário associará a ação correta ao efeito desejado?"]
    PC --> Q4["4. O usuário perceberá o progresso rumo ao objetivo?"]
```
