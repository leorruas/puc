...
## Avaliar comunicabilidade significa investigar a metacomunicação da interface
...
## Comunicabilidade mede a qualidade da mensagem de design
...
```mermaid
flowchart TD
    EngSem["Métodos de avaliação<br>da Engenharia Semiótica"] --> MIS["1. Método de<br>Inspeção Semiótica (MIS)<br>(Inspeção analítica da<br>mensagem pelo especialista)"]
    EngSem --> MAC["2. Método de<br>Avaliação da Comunicabilidade (MAC)<br>(Observação empírica da<br>recepção pelo usuário)"]
```
...
## MIS reconstrói a metamensagem por inspeção especializada
...
```mermaid
flowchart TD
    MIS["Etapas do Método<br>de Inspeção Semiótica (MIS)"] --> E1["1. Análise<br>dos signos estáticos<br>(Layout e elementos visuais)"]
    E1 --> E2["2. Análise<br>dos signos dinâmicos<br>(Comportamento e transições)"]
    E2 --> E3["3. Análise<br>dos signos metalinguísticos<br>(Ajuda e mensagens de erro)"]
    E3 --> S3["4. Reconstrução da meta-mensagem<br>do designer"]
    S3 --> E5["5. Avaliação da comunicabilidade<br>(Julgamento de coerência)"]
```

### Signos analisados no MIS
...
## MAC observa rupturas de comunicação durante o uso
...
```mermaid
flowchart TD
    MAC["Fluxo do Método de<br>Avaliação da Comunicabilidade (MAC)"] --> Teste["1. Teste de uso gravado<br>(Áudio, vídeo de tela<br>e rosto do usuário)"]
    Teste --> Tagging["2. Etiquetagem (Tagging)<br>(Associação de rupturas<br>às 13 expressões)"]
    Tagging --> Analise["3. Análise semiótica<br>(Identificação das<br>falhas de comunicabilidade)"]
```
...
### Treze expressões comunicativas do MAC
...
## MIS e MAC observam a comunicabilidade por evidências diferentes
