...
```mermaid
graph TD
    Critérios["Critérios ergonômicos de<br>Scapin e Bastien"] --> C1["1. Condução (Guidance)"]
    Critérios --> C2["2. Carga<br>cognitiva (Workload)"]
    Critérios --> C3["3. Controle<br>explícito (Explicit Control)"]
    Critérios --> C4["4.<br>Adaptabilidade (Adaptability)"]
    Critérios --> C5["5. Gestão de<br>erros (Error Management)"]
    Critérios --> C6["6.<br>Consistência (Consistency)"]
    Critérios --> C7["7. Significado<br>dos códigos (Significance)"]
    Critérios --> C8["8.<br>Compatibilidade (Compatibility)"]
```
...
```mermaid
graph TD
    Seleção["Decisão de componente<br>de seleção em formulários"] --> Q1{"Quantas opções o usuário pode escolher?"}
    Q1 -- "Apenas 1 (Exclusiva)" --> Q2{"Quantas alternativas existem no total?"}
    Q1 -- "0, 1 ou Várias (Múltipla)" --> Checkbox["Caixa de<br>seleção (Checkbox)"]
    Q2 -- "De 2 a 5 opções" --> Radio["Botões de<br>opção (Radio Button)"]
    Q2 -- "Mais de 5 opções" --> Dropdown["Lista suspensa<br>(Dropdown / Select)"]
    Q2 -- "Binário imediato (On/Off)" --> Toggle["Interruptor (Switch<br>/ Toggle)"]
```
...
```mermaid
graph TD
    Distância["Menor distância até<br>o alvo (D)"] --> Tempo["Tempo de<br>alcance reduzido (T)"]
    Tamanho["Maior área<br>do alvo (W)"] --> Tempo
```
