...
## Design combina conhecimento técnico e reflexão situada
...
## Racionalismo técnico trata problemas por métodos e regras
...
```mermaid
flowchart TD
    A["Problema Bem<br>Definido (Dado)"] --> B["Aplicação de Regras,<br>Modelos e Técnicas Científicas"]
    B --> C["Execução de Passos<br>Algorítmicos e Sistemáticos"]
    C --> D["Solução<br>Otimizada Racionalmente"]
```

### Premissas do racionalismo técnico
...
## Reflexão em ação responde a situações ambíguas enquanto o projeto acontece
...
```mermaid
flowchart TD
    A["Situação de Problema<br>Ambíguo (Wicked Problem)"] --> B["Enquadramento do<br>Problema (Problem Framing)"]
    B --> C["Ação /<br>Intervenção Experimental"]
    C --> D["Resposta da Situação<br>(Back-Talk do Material/Usuário)"]
    D --> E["Reflexão na<br>Ação (Reflection-in-Action)"]
    E -->|Reajuste de Hipóteses| B
```

### Premissas da reflexão em ação
...
## Técnica e criatividade são complementares no design de interação
...
flowchart TD
    ...
    Criatividade --> IxD["Design de<br>Interação de Excelência"]
```
...
## Comparação entre racionalismo técnico e reflexão em ação
...
## Exemplo integrado: redesign de aplicativo de mobilidade
...
## Problemas de design exigem método e capacidade de reenquadramento
...
- **Próximo artigo:** [[12. Modelagem de usuário - perfil, persona e mapa de empatia]]
