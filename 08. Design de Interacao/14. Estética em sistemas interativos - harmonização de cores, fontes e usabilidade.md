...
```mermaid
graph LR
    Círculo["Círculo cromático (12 cores)"] --> Esquemas["Esquemas de harmonização"]
    Esquemas --> Monocromático["Monocromático (Variação de tom)"]
    Esquemas --> Análogo["Análogo (Vizinhos)"]
    Esquemas --> Complementar["Complementar (Opostos)"]
```
