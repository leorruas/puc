...
```mermaid
graph TD
    A["Interface esteticamente harmoniosa e atraente"] --> B["Indução de afeto positivo e redução da ansiedade"]
    B --> C["Aumento da flexibilidade cognitiva do usuário"]
    C --> D["Percepção de que o sistema é mais fácil e agradável de usar"]
    D --> E["Maior tolerância a pequenas falhas temporárias de usabilidade"]
```
...
```mermaid
graph TD
    Tipografia["Escolhas tipográficas em UI"] --> H1["Hierarquia de tamanhos (Título > Subtítulo > Corpo)"]
    Tipografia --> H2["Altura de linha (Line-Height de 1.4 a 1.6x)"]
    Tipografia --> H3["Largura de parágrafo (45 a 75 caracteres por linha)"]
    Tipografia --> H4["Regra de pareamento (No máximo 2 famílias de fontes)"]
```
