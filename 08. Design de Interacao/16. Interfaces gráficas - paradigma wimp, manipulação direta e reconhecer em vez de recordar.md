...
flowchart TD
    ...
    WIMP --> I["Icons (Ícones)"]
    WIMP --> M["Menus (Menus)"]
    WIMP --> P["Pointer (Apontador<br>/ Cursor)"]
```
...
```mermaid
flowchart TD
    Objeto["Representação visual<br>contínua do objeto"] --> Ação["Ação física rápida<br>(Clique / Arrasto)"]
    Ação --> Feedback["Feedback visual<br>imediato e reversível"]
    Feedback --> Ajuste["Confirmação ou descarte<br>pelo usuário"]
```
...
```mermaid
flowchart TD
    subgraph Memoria ["Memória Cognitiva"]
        Recall["Recordar (Recall)<br>- Busca na<br>memória de longo prazo<br>- Alta carga cognitiva"]
        Recognition["Reconhecer (Recognition)<br>- Identificação<br>visual de pistas<br>- Baixa carga cognitiva"]
    end
```
