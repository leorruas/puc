...
flowchart TD
    subgraph Metacomunicacao ["Nível 2: Metacomunicação Designer-Usuário (Indireta)"]
        D["Designer (Emissor<br>da Visão)"] -->|Mensagem de Metacomunicação via Interface| U["Usuário (Receptor)"]
    end
    
    subgraph ComunicacaoDireta ["Nível 1: Comunicação Direta Usuário-Sistema"]
        U <-->|Interlocução em Tempo Real| S["Sistema (Preposto<br>do Designer)"]
    end
```
...
```mermaid
flowchart TD
    EC["Design Centrado<br>na Comunicabilidade"] --> MoLIC["MoLIC: Modelagem da<br>Interação como Conversa"]
    EC --> MAC["MAC: Avaliação da<br>Emissão da Metamensagem"]
    EC --> MIS["MIS: Inspeção da<br>Recepção da Metamensagem"]
```
