...
## O grau de participação do usuário muda o processo de design
...
## Projetar sem participação do usuário
...
## Inside-Out e Outside-In partem de pontos diferentes
...
| Abordagem | Ponto de partida | Direção típica |
| :--- | :--- | :--- |
| Inside-Out | Estrutura, lógica e restrições do software | Dados → lógica → interface → usuário |
| Outside-In | Necessidades, objetivos e contexto do usuário | Usuário → interface → lógica → dados |

### Inside-Out: partir da lógica do software
- **Direção do fluxo:** parte do centro (camadas internas da aplicação) em direção às bordas (usuários externos).
- **Caminho de projeto:** **Dados → Lógica → Interface → Usuários**.
...
### Outside-In: partir das necessidades do usuário
...
- **Caminho de projeto:** **Usuários → Interface → Lógica → Dados**.
...
## Design centrado no usuário como processo iterativo
...
### Ciclo de vida do design de interação
...
```mermaid
flowchart TD
    Necessidades["Identificar necessidades<br>e definir requisitos"]:::external
    Design["(Re)design"]:::core
    Prototipo["Construir versão interativa"]:::component
    Avaliar["Avaliar com usuários e critérios"]:::data
    Produto["Produto / solução em uso"]:::component

    Necessidades --> Design
    Design --> Prototipo
    Prototipo --> Avaliar
    Avaliar -->|"novos requisitos"| Necessidades
    Avaliar -->|"ajustes de design"| Design
    Avaliar -->|"critérios atendidos"| Produto
```

#### Quatro atividades básicas de IxD
...
## Design participativo e co-design envolvem o usuário na criação
...
### Projetar com o usuário, não apenas para ele
...
### *Design by Doing*: aprender e projetar fazendo
...
## Comparação entre modos de envolvimento do usuário
...
## Exemplo integrado: prontuário eletrônico em postos de saúde
...
## Participação do usuário altera conhecimento, poder e decisão no projeto

- **Projetar de modo independente (Inside-Out):** foco no software (Dados → Lógica → Interface → Usuários), forçando o usuário a *"aprender a usar o sistema como ele é"* e *"aceitar suas limitações"*.
- **Projetar centrado no usuário (UCD / Outside-In):** foco no usuário (Usuários → Interface → Lógica → Dados), seguindo o **ciclo de vida iterativo de IxD** (requisitos → redesign → versão interativa → avaliar).
...
- **Próximo artigo:** [[11. Racionalismo técnico e reflexão em ação - o papel da técnica e da criatividade no design]]
