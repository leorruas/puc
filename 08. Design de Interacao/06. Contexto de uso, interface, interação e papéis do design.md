...
graph TD
    subgraph UX ["UX Design (User Experience)"]
        direction TD
        A["Estratégia, pesquisa, proposta de valor e jornada do usuário"] --> B
        
        subgraph IA ["IA (Arquitetura da Informação)"]
            direction TD
            B["Estrutura de conteúdo, navegação, taxonomias e busca"] --> C
            
            subgraph IxD ["IxD (Design de Interação)"]
                direction TD
                C["Comportamento, fluxos de uso e respostas do sistema"] --> D
                
                subgraph UI ["UI Design (User Interface)"]
                    D["Camada de apresentação visual, estética e componentes gráficos"]
                end
            end
        end
    end
