...
* **No software:** Uma **classe abstrata** funciona exatamente como esse molde incompleto. Ela define a estrutura comum e as regras de negócio compartilhadas, mas **proíbe o compilador de criar instâncias diretas (`new`)**, forçando subclasses concretas a preencherem as partes que faltam.
...
2. **Falta de obrigatoriedade na especialização:**
   - Um método `virtual` fornece uma implementação padrão opcional. Se a subclasse esquecer de dar `override`, o código compila normalmente. Em contrapartida, um **método abstrato** transforma a especialização em uma **obrigação contratual inegociável**: se a subclasse não implementar o método, ela não compila!
...
Utiliza o modificador `abstract` antes da palavra-chave `class`:
