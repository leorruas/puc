...
relacionados:
  ...
  - "[[00. Sintaxe Multilinguagem/03. Estruturas de repetição (while, for, do-while)]]"
---
...
## Programação modular como estratégia de decomposição
...
## Algoritmos e a construção de software
...
Antes de estruturar sistemas em módulos complexos, a lógica fundamental de qualquer programa baseia-se na combinação de três construções lógicas primitivas de controle de fluxo (veja a sintaxe prática em [[00. Sintaxe Multilinguagem/00. Guia multilinguagem de sintaxe e praticas - Índice|Guia Multilinguagem de Sintaxe]]):

* **Comandos sequenciais:** instruções executadas de forma linear, uma após a outra, na ordem exata em que foram escritas → [[00. Sintaxe Multilinguagem/01. Variáveis, tipos e atribuição|Sintaxe de Variáveis e Atribuição]].
* **Comandos de seleção (decisão):** estruturas que alteram o fluxo de execução com base em condições lógicas (`if/else`, `switch/case`) → [[00. Sintaxe Multilinguagem/02. Estruturas condicionais (if, else, switch)|Sintaxe de Condicionais]].
* **Comandos de repetição (laços/iterações):** estruturas que permitem a execução reiterada de um bloco de código enquanto uma condição for satisfeita (`while`, `for`, `do-while`) → [[00. Sintaxe Multilinguagem/03. Estruturas de repetição (while, for, do-while)|Sintaxe de Repetição]].
...
Dessa forma, provou-se que o `GOTO` não é estritamente necessário para expressar nenhuma lógica computável → [[00. Sintaxe Multilinguagem/02. Estruturas condicionais (if, else, switch)#O caso histórico do GOTO (Salto incondicional descontinuado)|Entenda a mecânica e a descontinuação do GOTO no Guia de Sintaxe]].
...
## Três etapas da evolução da abstração no software

A construção de softwares escaláveis seguiu uma progressão lógica de abstração em três níveis:

```mermaid
flowchart TD
    A["1. Decomposição<br>(Dividir o algoritmo em funções e sub-rotinas)"]
    B["2. Modularização<br>(Agrupar funções e dados em módulos/arquivos isolados)"]
    C["3. Gestão do próprio estado<br>(Encapsulamento e TADs: o módulo gerencia seus dados)"]

    A --> B
    B --> C
```

1. **Decomposição (*Top-Down*):** consiste em quebrar um problema grande em sub-problemas menores resolvidos por funções isoladas → [[00. Sintaxe Multilinguagem/04. Sub-rotinas (funções e procedimentos)|Sintaxe de Sub-rotinas]].
2. **Modularização:** consiste em agrupar as funções e estruturas relacionadas em unidades de compilação ou arquivos separados, reduzindo acoplamento e evitando duplicação $→ [[00. Sintaxe Multilinguagem/06. Modificadores de acesso, escopo e a palavra-chave static|Sintaxe de Módulos e Cabeçalhos]].
3. **Gestão do próprio estado (*Encapsulamento de Estado*):** estágio em que o módulo deixa de ser apenas um pacote de funções soltas e passa a **possuir, proteger e gerenciar seu próprio estado interno** (dados e variáveis) → [[00. Sintaxe Multilinguagem/05. Abstração e encapsulamento com classes (TADs)|Sintaxe de Classes e Encapsulamento]].

---

## Evolução histórica da programação e dos algoritmos

```mermaid
flowchart TD
    A["Código de máquina e Assembly<br>(Comandos sequenciais e saltos com GOTO)"]
    B["Programação estruturada<br>(Sequência, seleção, repetição e funções)"]
    C["Tipos abstratos de dados e módulos<br>(Modularização de código e interface pública)"]
    D["Orientação a objetos e pacotes<br>(Gestão do próprio estado, classes e métodos)"]
    E["Arquiteturas modulares modernas<br>(Microsserviços, componentes e APIs)"]
...
## Principais elementos de programação para escalabilidade

| Elemento | Descrição | Papel na escalabilidade | Guia de Sintaxe |
| --- | --- | --- | --- |
| **Comandos estruturados** | Estruturas sequenciais, de seleção e repetição | Dão previsibilidade ao fluxo algorítmico | [[00. Sintaxe Multilinguagem/02. Estruturas condicionais (if, else, switch)\|Sintaxe]] |
| **Funções (decomposição)** | Blocos isolados que executam uma tarefa específica | Isolam a complexidade instrucional | [[00. Sintaxe Multilinguagem/04. Sub-rotinas (funções e procedimentos)\|Sintaxe]] |
| **Módulos (modularização)** | Agrupamento de funções e variáveis em arquivos | Organizam o projeto e delimitam escopos | [[00. Sintaxe Multilinguagem/06. Modificadores de acesso, escopo e a palavra-chave static\|Sintaxe]] |
| **TADs / Encapsulamento** | Ocultamento da implementação e proteção do estado | Protegem a integridade dos dados | [[00. Sintaxe Multilinguagem/05. Abstração e encapsulamento com classes (TADs)\|Sintaxe]] |
| **Namespaces / Pacotes** | Agrupamento hierárquico | Evitam conflitos de identificadores | [[00. Sintaxe Multilinguagem/06. Modificadores de acesso, escopo e a palavra-chave static\|Sintaxe]] |
| **Interfaces / Contratos** | Assinaturas de métodos sem expor detalhes | Garantem desacoplamento entre componentes | [[00. Sintaxe Multilinguagem/05. Abstração e encapsulamento com classes (TADs)\|Sintaxe]] |

---

## Objetivos e benefícios da programação modular
