...
* **[[Glossário de conceitos#Objeto Instance|Objeto / Instância (Instance)]]:** Uma **instância** é a materialização viva, real e concreta de uma classe criada na memória RAM. O processo de criar esse objeto na memória chama-se **Instanciação** (`new`). Você pode criar múltiplas instâncias a partir de uma mesma classe, onde cada instância possui sua própria posição de memória e estado individual.
...
* **[[Glossário de conceitos#8 Declaração em Computação Declaration Declarar|Declaração (Declaration)]]:** O ato técnico de apresentar os nomes e tipos dos membros ao compilador (`double _saldo;`), diferenciando a reserva de identidade da sua inicialização e instanciação em memória.
...
flowchart LR
    ...
    subgraph Heap ["Memória Heap (Área Dinâmica)"]
        direction TB
        Obj["Objeto Real na Memória<br>new ContaCorrente(...)<br>Endereco: 0x7FFE<br>- _numero: '1001-X'<br>- _titular: 'Leo Ruas'<br>- _saldo: 500.00"]
    end
...
## 6. Diferenças conceituais: atributos vs. variáveis locais
