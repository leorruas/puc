...
```mermaid
flowchart LR
    Classe["Classe Carro<br>define estrutura e comportamento"]:::core
    New["new"]:::component
    Vermelho["Objeto 1<br>estado próprio"]:::data
    Preto["Objeto 2<br>estado próprio"]:::data

    Classe -->|"instancia com"| New
    New --> Vermelho
    New --> Preto
```
...
flowchart TD
    ...
    Estado["Estado privado<br>_numeroConta, _titular, _saldo"]:::data
    Metodos["Métodos públicos<br>Depositar, Sacar, ObterSaldo"]:::component

    Conta --> Estado
    Conta --> Metodos
    Metodos -->|"protegem regras sobre"| Estado
```
...
```mermaid
flowchart LR
    Argumento["Argumento<br>valor enviado pela chamada"]:::external
    Parametro["Parâmetro<br>recebe durante a execução"]:::component
    Campo["Campo de instância<br>mantém estado do objeto"]:::data

    Argumento --> Parametro
    Parametro -->|"atribuição"| Campo
```
...
```mermaid
flowchart LR
    Referencia["Variável de referência<br>ContaCorrente conta"]:::data
    Objeto["Objeto ContaCorrente<br>estado da instância"]:::core

    Referencia -->|"referencia"| Objeto
```
...
flowchart LR
    New["new"]:::core
    Alocacao["Runtime cria a instância"]:::component
    Construtor["Construtor inicializa o estado"]:::component
    Referencia["Referência resultante<br>é atribuída à variável"]:::data
...
| Modificador | Acesso principal |
| :--- | :--- |
| `public` | Onde o membro e seu tipo forem acessíveis |
| `protected` | No tipo e em tipos derivados, conforme as regras da linguagem |
| `private` | Dentro do tipo que declara o membro |
...
| Prática | Objetivo |
| :--- | :--- |
| Campos privados | Evitar alteração livre do estado interno |
| Métodos coesos | Concentrar cada operação em uma responsabilidade clara |
| Validar parâmetros | Impedir transições inválidas de estado |
| Ocultar implementação | Expor contrato sem obrigar o cliente a conhecer detalhes internos |
