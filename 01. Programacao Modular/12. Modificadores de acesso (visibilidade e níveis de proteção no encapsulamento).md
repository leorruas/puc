...
> Tipos em nível de namespace só podem ser **`public`** ou **`internal`** (o padrão implícito). O modificador `private` em classes só é válido para **classes aninhadas (*Nested Classes*)**, declaradas dentro do corpo de outra classe.
...
## 7. Constantes e visibilidade: a relação entre `const` e `public`

Uma das regras de ouro do encapsulamento afirma que *"campos de instância nunca devem ser `public`"*. No entanto, existe uma **exceção consagrada na engenharia de software**: os campos marcados como **`public const`**.
...
| :--- | :--- | :--- |
| **Momento de atribuição** | **Tempo de compilação** (*Compile-time*). O valor deve ser um literal numérico, string ou booleano. | **Tempo de execução** (*Runtime*). Pode ser calculado no construtor estático. |
| **Tipos aceitos** | Tipos primitivos (`int`, `double`, `bool`, `string`). | Qualquer tipo de objeto (ex.: `DateTime`, instâncias de classes). |
...
| **Exemplo típico** | `public const double TaxaIOF = 0.0038;` | `public static readonly DateTime DataLancamento = DateTime.Now;` |

## 8. O paradoxo do construtor privado: para que serve uma classe que não pode ser instanciada?
...
flowchart TD
    subgraph ConstrutorPrivado ["Aplicações de Classes com Construtor Privado"]
        ...
        subgraph CasosDeUso ["Dois Grandes Padrões de Projeto"]
            StaticUtil["1. Classes Utilitárias Estáticas (Static Utility Classes)<br>- Agrupam apenas métodos e constantes puras.<br>- Não guardam estado de instância.<br>- Ex.: Math, Console, File, Convert."]
...
### 1. Classes utilitárias estáticas (*Static Utility Classes*)

Uma **classe utilitária estática** é um módulo coeso cujo único propósito é fornecer um **conjunto de operações puras, funções matemáticas ou rotinas utilitárias** que não dependem de estado de objeto individual.
...
* **A evolução no C# (`static class`):** 
  - Em C# clássico (ou em Java), para impedir a instanciação, o desenvolvedor criava um construtor privado explícito:
    ...
    ```
  - A partir do C# 2.0, a linguagem introduziu a palavra-chave **`static class`**, que faz o compilador gerar automaticamente o construtor privado por debaixo dos panos, impedindo a criação de membros não estáticos e travando qualquer tentativa de `new`.
...
## 10. Resumo comparativo: escolha do modificador correto
