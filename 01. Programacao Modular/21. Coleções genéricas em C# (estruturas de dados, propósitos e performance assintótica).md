...
```mermaid
flowchart TD
    Colecoes["Coleções genéricas"]:::core
    Sequenciais["Sequenciais"]:::component
    Associativas["Associativas"]:::data
    Conjuntos["Conjuntos"]:::data
    List["List<T>"]:::component
    Queue["Queue<T>"]:::component
    Stack["Stack<T>"]:::component
    Linked["LinkedList<T>"]:::component
    Dict["Dictionary<TKey, TValue>"]:::data
    Set["HashSet<T>"]:::data

    Colecoes --> Sequenciais
    Colecoes --> Associativas
    Colecoes --> Conjuntos
    Sequenciais --> List
    Sequenciais --> Queue
    Sequenciais --> Stack
...
| Namespace | Exemplos | Uso principal |
| :--- | :--- | :--- |
| `System.Collections` | `ArrayList`, `Hashtable` | APIs legadas não genéricas |
| `System.Collections.Generic` | `List<T>`, `Dictionary<TKey,TValue>`, `HashSet<T>` | Coleções tipadas de uso geral |
| `System.Collections.Concurrent` | `ConcurrentDictionary<K,V>`, `ConcurrentQueue<T>` | Cenários de acesso concorrente com operações thread-safe próprias |
...
```mermaid
flowchart LR
    A["A<br>coleção imutável"]:::data
    Add["B = A.Add(40)"]:::core
    A2["A permanece inalterada"]:::data
    B["B é uma nova versão<br>com compartilhamento estrutural"]:::component
...
```mermaid
flowchart TD
    Q1{"Acesso principal por chave?"}:::core
    Q2{"Precisa manter chaves ordenadas?"}:::core
    Dict["Dictionary<TKey, TValue>"]:::data
    Sorted["SortedDictionary<TKey, TValue>"]:::data
    Q3{"Elementos devem ser únicos?"}:::core
    Set["HashSet<T>"]:::data
    Q4{"Ordem de processamento é fixa?"}:::core
    Queue["Queue<T><br>FIFO"]:::component
    Stack["Stack<T><br>LIFO"]:::component
    List["List<T><br>acesso posicional geral"]:::component
