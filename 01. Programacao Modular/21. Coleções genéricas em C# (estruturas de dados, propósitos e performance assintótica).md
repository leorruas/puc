...
# Coleções genéricas em C#: estruturas de dados, propósitos e análise de performance
...
## Coleções genéricas e escolha de estrutura de dados
...
### Intuição: estruturas diferentes para operações diferentes
...
### `Generic`, `Concurrent` e `Immutable` em comparação
...
### Por que `System.Collections` perdeu espaço no código moderno
...
### Coleções concorrentes e acesso por múltiplas threads
...
### Coleções imutáveis e persistência estrutural
...
#### Compartilhamento estrutural entre versões
...
#### Intuição: versões persistentes em vez de mutação
...
#### Exemplo de coleção imutável em C#
```csharp
using System;
...
#### Cenários adequados para imutabilidade
...
### Complexidade de coleções mutáveis e imutáveis
...
#### Por que operações imutáveis tendem a custar $O(\log N)$
...
## Estruturas fundamentais e operações

---

### `List<T>`: array dinâmico contíguo
...
### `Dictionary<TKey, TValue>`: mapa associativo por hash
...
#### Relação conceitual entre objetos JSON e pares chave-valor
...
### Operações principais por coleção
...
## Complexidade assintótica das principais operações
...
## Critérios de escolha entre coleções
...
## Coleções em um fluxo integrado de transações
