...
```mermaid
flowchart TD
    Estatico["TaxaJuros<br>campo static compartilhado"]:::core
    Leo["Conta Leo<br>estado próprio"]:::data
    Ana["Conta Ana<br>estado próprio"]:::data
    Carlos["Conta Carlos<br>estado próprio"]:::data

    Leo -.->|"consulta"| Estatico
    Ana -.->|"consulta"| Estatico
    Carlos -.->|"consulta"| Estatico
```
...
| Membro `static` | Papel |
| :--- | :--- |
| Campo estático | Estado associado ao tipo |
| Método estático | Operação chamada sem instância |
| Propriedade estática | Acesso encapsulado a estado ou comportamento do tipo |
| Construtor estático | Inicialização do tipo controlada pelo runtime |
...
| Parte de `Main` | Papel |
| :--- | :--- |
| `static` | Permite usar o ponto de entrada sem criar uma instância de `Program` |
| `void` ou `int` | Define a forma de retorno aceita pelo ponto de entrada tradicional |
| `Main` | Nome reconhecido como ponto de entrada conforme as assinaturas válidas |
| `string[] args` | Recebe argumentos da linha de comando quando essa assinatura é usada |
...
```mermaid
flowchart LR
    Contador["static int _proximoId"]:::core
    Construtor["new Cliente()"]:::component
    Id["_id recebe o valor atual"]:::data
    Incremento["_proximoId++"]:::component

    Contador --> Construtor
    Construtor --> Id
    Id --> Incremento
    Incremento -->|"atualiza"| Contador
```
...
| Cuidado com `static` | Consequência possível |
| :--- | :--- |
| Estado mutável global | Dependências ficam menos explícitas |
| Compartilhamento entre testes | Um teste pode influenciar outro |
| Concorrência | Atualizações podem exigir sincronização |
| Uso adequado | Constantes, utilidades e estado realmente pertencente ao tipo |
...
```mermaid
flowchart TD
    Propriedade["Saldo"]:::core
    Get["get<br>lê _saldo"]:::component
    Set["set<br>valida antes de alterar"]:::component
    Campo["_saldo"]:::data
...
| Forma de propriedade | Característica |
| :--- | :--- |
| Completa | Usa campo de apoio e lógica explícita em `get`/`set` |
| Auto-property | O compilador fornece o armazenamento associado |
| Somente leitura / escrita restrita | Limita quem pode alterar o valor com `get`, `init` ou setter menos acessível |
| Estática | Pertence ao tipo, não a cada instância |
