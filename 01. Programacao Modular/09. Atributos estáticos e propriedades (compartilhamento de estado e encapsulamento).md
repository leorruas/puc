...
2. **Métodos estáticos não possuem `this`:** Como o método estático roda no escopo da classe e não em uma instância específica, ele **não tem acesso ao ponteiro `this`** e não pode ler ou alterar diretamente atributos de instância.
3. **Barreira de acesso a atributos de instância (A pegadinha clássica `Jump()` e `count`):**
   - Para que um método estático possa acessar ou modificar o estado de um objeto, ele é **terminantemente obrigado a receber a referência desse objeto como parâmetro**:
   ```csharp
   ...
   ```
4. **Compartilhamento em tempo real:** Se uma instância alterar um atributo estático público ou através de um método, todas as outras instâncias verão a alteração no exato milissegundo seguinte.
