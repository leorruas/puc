...
* **Atributo estático (`static`):** O **ar-condicionado ou a luz do teto da sala de aula**. Ele não pertence a nenhum aluno individualmente, mas sim ao **espaço compartilhado da sala (a Classe)**. Se qualquer pessoa mudar a temperatura do ar-condicionado para 20°C, **todos os alunos na sala sentem a nova temperatura instantaneamente**.

---

## 2. Membros estáticos: atributos, métodos e construtores

A palavra-chave `static` indica que o membro pertence à **Classe como um todo**, e não a um objeto instanciado individualmente.

```mermaid
flowchart TD
    subgraph MembrosEstaticos ["Taxonomia dos Membros Estáticos"]
        direction TB
