...
#### 3. Convenções consagradas de nomenclatura:
* Se for apenas um parâmetro genérico, utilize a letra única **`T`** (ex.: `List<T>`).
* Se houver múltiplos parâmetros genéricos, utilize nomes descritivos iniciados pelo prefixo `T` (ex.: `TKey`, `TValue`, `TRequest`, `TResponse`, `TInput`, `TOutput`).
