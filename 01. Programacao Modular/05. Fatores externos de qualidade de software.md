...
* **Definição:** É a capacidade do sistema de **reagir de forma aceitável e segura diante de situações anormais ou imprevistas**, como entradas inválidas, queda de banco de dados ou arquivos corrompidos, sem sofrer travamentos catastróficos (*crashes*).
* **Analogia de Feynman:** Um filtro de linha ou airbag de um carro. Se houver um pico de energia ou buraco na pista, o carro não pode simplesmente desligar o motor na rodovia; ele deve acionar os mecanismos de proteção e manter o passageiro seguro.
* **Como se aplica em código:** Uso correto de tratamento de exceções (`try / catch / finally`), validação de dados de entrada e tolerância a falhas.
...
```mermaid
flowchart TD
    subgraph ISO25010 ["Fatores de Qualidade de Produto (ISO/IEC 25010)"]
        direction TB
        ...
        C4["4. Usabilidade<br>(Aprendizado, Uso, Proteção a Erros, Estética, Acessibilidade)"]
        C5["5. Confiabilidade<br>(Maturidade, Disponibilidade, Tolerância a Falhas, Recuperabilidade)"]
        C6["6. Segurança<br>(Confidencialidade, Integridade, Não-Repúdio, Autenticidade, Responsabilidade)"]
        C7["7. Manutenibilidade<br>(Modularidade, Reusabilidade, Analisabilidade, Modificabilidade, Testabilidade)"]
