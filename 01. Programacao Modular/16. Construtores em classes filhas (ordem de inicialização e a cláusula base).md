...
# Construtores em classes filhas (ordem de inicialização e a cláusula base)
...
## Inicialização de classes derivadas

Construir uma subclasse envolve inicializar primeiro a parte base e depois a parte derivada. A cláusula `base(...)` torna essa ordem explícita.

### Classe base antes da classe derivada
...
## Cadeia de construtores com `base(...)` e `this(...)`

Leia as chamadas `: this(...)` e `: base(...)` como encadeamentos diferentes. O exemplo completo mostra a ordem de execução e o risco de chamar métodos virtuais cedo demais.

### Construtores pertencem à classe que inicializam
...
**Razões para construtores não serem herdados.**
...
### Encaminhamento de argumentos com `base(...)`
...
### Exemplo de inicialização de base e derivada em C#
...
### Classe base sem construtor sem parâmetros

Existem dois cenários cruciais de compilação:

**Base com construtor sem parâmetros.**
...
**Base apenas com construtores parametrizados.**
...
### Encadeamento interno com `this(...)` e ancestral com `base(...)`
...
public class Caminhao : Veiculo
{
    ...
    public Caminhao(string placa, string modelo, int anoFabricacao)
        : this(placa, modelo, anoFabricacao, 8.0, 2) // Assume 8 toneladas e 2 eixos por padrão
    {
    }
}
```
...
### Construtores e garantia de estado válido

> [!NOTE]
> **Reflexão sobre qualidade e invariantes:**
> Os construtores, junto com o encapsulamento, são o **1º passo para a garantia da correção e robustez de um sistema** (veja [[05. Fatores externos de qualidade de software|fatores externos de qualidade]]). Por isso, em herança, o programador deve sempre implementá-los e propagar os argumentos para o construtor da classe base.
>
> Mas, dependendo da regra de negócio, os construtores da classe filha podem:
> 1. Chamar construtores *default* (padrão) na classe pai; ou
> 2. Passar parâmetros *default* fixos para a classe pai que não foram recebidos do usuário (ex.: criar uma `ContaUniversitaria` que sempre repassa `limiteChequeEspecial = 0m` e `tarifa = 0m` para a classe pai).

**Responsabilidade da classe base sobre suas invariantes.**

**SIM, desde que a classe base tenha sido projetada com alto encapsulamento!**

1. **A classe base é a guardiã suprema de suas próprias invariantes:** Mesmo que a classe filha passe valores padrão fixos através de `: base(...)`, o construtor da classe base **ainda executará suas próprias validações internas** (como `if (saldo < 0) throw ...`). Se o valor padrão fornecido pela filha violar uma invariante da base, o programa lançará uma exceção imediatamente, impedindo a criação de um objeto em estado corrompido.
2. **O perigo de expor construtores *default* vazios (`public Pai() { }`):** Se a classe pai permitir a criação de instâncias sem parâmetros e deixar atributos essenciais com valor `null` ou `0`, a **correção do sistema é comprometida**. Por isso, a boa prática de engenharia exige que a classe base **torne obrigatórios todos os dados indispensáveis** através de construtores parametrizados, forçando as classes filhas a supri-los (seja via argumentos recebidos ou valores padrão de negócio consistentes).

---

## Métodos virtuais durante a construção do objeto

Construtores não são herdados; inicializadores de campos, construtor base e construtor derivado têm momentos próprios. A localização física em Stack/Heap é apenas um modelo didático.

### Risco de chamar métodos virtuais no construtor

> [!CAUTION]
> **Armadilha clássica de arquitetura:** Evite invocar métodos virtuais (`virtual`) de dentro de um construtor.

**Estado parcialmente inicializado e despacho virtual.**
1. Quando o construtor da superclasse está rodando, o despacho de um método virtual pode alcançar a implementação mais derivada, conforme as regras do runtime.
...
### Exemplo integrado da ordem de inicialização
    ...
    public class ContaBancaria
    {
        ...
        public ContaBancaria(string numero, string titular, decimal saldoInicial)
        {
            if (string.IsNullOrWhiteSpace(numero))
                throw new ArgumentException("Número da conta é necessário quando a classe base não possui um construtor sem parâmetros acessível.", nameof(numero));

            if (string.IsNullOrWhiteSpace(titular))
                throw new ArgumentException("Nome do titular é necessário quando a classe base não possui um construtor sem parâmetros acessível.", nameof(titular));
    ...
    public class ContaCorrenteEmpresarial : ContaBancaria
    {
            ...
            : base(numero, titular, saldoInicial)
        {
            if (string.IsNullOrWhiteSpace(cnpj))
                throw new ArgumentException("CNPJ da empresa é necessário quando a classe base não possui um construtor sem parâmetros acessível.", nameof(cnpj));
...
## Boas práticas em hierarquias de construção

A inicialização conecta herança ao artigo 15 e prepara o despacho de métodos do artigo 17. Composição pode evitar parte desses acoplamentos.

### Regras para construtores de classes base e derivadas
...
### Construtores em herança e composição

> [!NOTE]
> **Esclarecimento conceitual:** Todo o conteúdo deste artigo trata **estritamente do mecanismo de herança (*Inheritance*)**.

A cláusula **`: base(...)`** é um recurso sintático e arquitetural exclusivo da herança de classes. Veja o contraste fundamental entre os dois modelos:

| Critério | Neste artigo 16: **Herança (*Inheritance*)** | Se o design usasse **Composição (*Composition*)** |
| :--- | :--- | :--- |
| **Declaração** | `public class Caminhao : Veiculo` | `public class Caminhao` (sem dois-pontos) |
| **Relação semântica** | **"É um" (*Is-A*)**: o caminhão **é um** veículo. | **"Tem um" (*Has-A*)**: o caminhão **tem um** motor (`_motor`). |
| **Papel do construtor** | Usa `: base(...)` para delegar a inicialização do pai antes do corpo da filha rodar. | Instancia ou recebe objetos internos por injeção (`_motor = motor;`) dentro do corpo `{ }`. |
| **Estrutura na memória** | Um **único objeto unificado** no *Heap* contendo os dados do pai e do filho. | Múltiplos objetos distintos na memória conectados por ponteiros e referências. |

Para entender quando e por que escolher composição em vez de herança, consulte o [[15. Herança (generalização, especialização e extensibilidade modular)#Por que favorecer composição sobre herança? (Favor Composition Over Inheritance)|Artigo 15 (Seção 10: Favorecer composição sobre herança)]].
...
* **Próximo artigo:** [[17. Sobreposição de métodos (virtual e override)]]
