...
```mermaid
flowchart LR
    Entrada["Entrada x"]:::data
    Funcao["f<br>delegate ou lambda"]:::core
    Saida["Resultado f(x)"]:::data
...
```mermaid
flowchart LR
    Contrato["CalculadoraTaxa<br>delegate decimal(decimal)"]:::core
    Pix["TaxaPix"]:::component
    Credito["TaxaCredito"]:::component
    Variavel["calc"]:::data
    Resultado["calc(1000m)"]:::component

    Pix -.->|"assinatura compatível"| Contrato
    Credito -.->|"assinatura compatível"| Contrato
    Contrato -->|"tipa"| Variavel
    Variavel -->|"invoca método atribuído"| Resultado
```
...
```mermaid
flowchart LR
    Tipo["OperacaoMatematica<br>delegate int(int, int)"]:::data
    Lambda["(a, b) => a * b"]:::component
    Atribuicao["OperacaoMatematica op = ..."]:::core

    Tipo -->|"define assinatura"| Atribuicao
    Lambda -->|"fornece implementação"| Atribuicao
```
...
```mermaid
flowchart LR
    C1["C# 1<br>método nomeado"]:::component
    C2["C# 2<br>função anônima"]:::component
    C3["C# 3+<br>lambda"]:::core

    C1 -->|"reduz boilerplate"| C2
    C2 -->|"sintaxe mais concisa"| C3
```
...
| Tipo | Retorno | Uso típico |
| :--- | :--- | :--- |
| `Action<...>` | `void` | Ações e efeitos colaterais |
| `Func<..., TResult>` | `TResult` | Cálculos e transformações |
| `Predicate<T>` | `bool` | Testes, busca e validação |
...
```mermaid
flowchart TD
    Q1{"Você precisa apenas passar<br>uma função com assinatura comum?"}:::core
    Padrao["Use Func / Action / Predicate"]:::component
    Q2{"O contrato precisa de nome de domínio<br>ou assinatura que os delegates padrão não expressam bem?"}:::core
    Custom["Declare delegate customizado"]:::component
    Evento["Para eventos, prefira o padrão<br>EventHandler / EventHandler<TEventArgs>"]:::data

    Q1 -->|"sim"| Padrao
    Q1 -->|"não"| Q2
    Q2 -->|"sim"| Custom
    Q2 -->|"é um evento"| Evento
```
...
```mermaid
flowchart LR
    Publicador["Publicador<br>declara o evento"]:::core
    Assinantes["Assinantes<br>registram handlers com +="]:::component
    Disparo["Publicador<br>invoca o evento"]:::core
    Handlers["Handlers inscritos<br>são chamados"]:::component

    Publicador --> Assinantes
    Assinantes --> Disparo
    Disparo --> Handlers
```
...
```mermaid
flowchart TD
    Evento["PedidoAprovado"]:::core
    Estoque["BaixarEstoque()"]:::component
    Fiscal["EmitirNota()"]:::component
    Email["EnviarComprovante()"]:::component
    Mensagem["NotificarCliente()"]:::external
