...
```mermaid
flowchart TD
    SoC["Separação de preocupações"]:::core
    UI["Apresentação"]:::component
    Dominio["Domínio"]:::component
    Persistencia["Persistência"]:::data
    Infra["Infraestrutura"]:::external
...
flowchart TD
    SOLID["SOLID"]:::core
    SRP["SRP<br>responsabilidade única"]:::component
    OCP["OCP<br>aberto / fechado"]:::component
    LSP["LSP<br>substituição"]:::component
    ISP["ISP<br>segregação de interfaces"]:::component
    DIP["DIP<br>inversão de dependência"]:::component
...
```mermaid
flowchart LR
    CFO["Financeiro"]:::external
    DBA["Dados"]:::external
    UX["UX / apresentação"]:::external
    Folha["FolhaDePagamento<br>3 motivos de mudança"]:::warning

    CFO -->|"altera cálculo"| Folha
    DBA -->|"altera persistência"| Folha
    UX -->|"altera saída"| Folha
```
...
```mermaid
flowchart LR
    Monolito["RelatorioFinanceiro<br>múltiplas responsabilidades"]:::warning
    Calculo["CalculadoraLucro"]:::component
    Repositorio["RepositorioFinanceiro"]:::data
    Relatorio["GeradorRelatorioPdf"]:::component

    Monolito -->|"separar regra"| Calculo
    Monolito -->|"separar dados"| Repositorio
    Monolito -->|"separar apresentação"| Relatorio
```
...
```mermaid
flowchart LR
    AnuncioMisto["Anuncio<br>estado + análise estatística"]:::warning
    Anuncio["Anuncio<br>estado do informe"]:::component
    Analisador["AnalisadorCampanhaPublicitaria<br>alcance e ROI"]:::component

    AnuncioMisto -->|"separa responsabilidades"| Anuncio
    AnuncioMisto -->|"separa responsabilidades"| Analisador
```
...
```mermaid
flowchart LR
    CheckoutDireto["MotorDeCheckout<br>acoplamento direto"]:::warning
    Concretos["Cartão / Boleto / Pix / PayPal"]:::warning
    Checkout["MotorDeCheckout"]:::core
    Contrato["IPagamento"]:::data
    Plugins["Formas de pagamento"]:::component

    CheckoutDireto -->|"depende de"| Concretos
    Checkout -->|"depende de"| Contrato
    Plugins -.->|"implementam"| Contrato
```
...
```mermaid
flowchart TD
    LSP["Violação de LSP"]:::warning
    Taxonomia["Confundir 'é um'<br>com contrato comportamental"]:::component
    Compilador["Confiar só no compilador<br>para validar semântica"]:::component
    Contrato["Fortalecer pré-condições<br>ou enfraquecer pós-condições"]:::component
...
```mermaid
flowchart LR
    Contrato["Contrato de Retangulo<br>largura e altura independentes"]:::core
    Quadrado["Quadrado como subtipo<br>força lados iguais"]:::warning
    Cliente["Código cliente<br>espera área 50"]:::external
    Resultado["Área obtida 100"]:::warning

    Cliente -->|"usa"| Contrato
    Contrato -->|"substituído por"| Quadrado
    Quadrado -->|"quebra expectativa"| Resultado
```
...
```mermaid
flowchart TD
    ISP["Interface monolítica<br>métodos irrelevantes"]:::warning
    SRP["Afeta SRP<br>múltiplos motivos de mudança"]:::component
    LSP["Afeta LSP<br>operações não suportadas"]:::component
    OCP["Afeta OCP<br>mudanças propagadas"]:::component
    DIP["Afeta DIP<br>abstração poluída"]:::component
...
```mermaid
flowchart TD
    Contrato["IRepositorioCliente<br>Salvar(Cliente)<br>BuscarPorId(Guid)"]:::core
    Servico["Regra de negócio"]:::component
    Sql["RepositorioSql"]:::data
    Mongo["RepositorioMongo"]:::data
    API["RepositorioApi"]:::external

    Servico -->|"depende do contrato"| Contrato
    Sql -.->|"implementa"| Contrato
    Mongo -.->|"implementa"| Contrato
    API -.->|"implementa"| Contrato
```
...
```mermaid
flowchart LR
    ServicoDireto["ServicoVendas<br>alto nível"]:::warning
    BancoDireto["SqlServerDatabase<br>baixo nível"]:::warning
    Servico["ServicoVendas"]:::core
    Contrato["IRepositorioVendas"]:::data
    Banco["SqlServerDatabase"]:::component

    ServicoDireto -->|"new direto"| BancoDireto
    Servico -->|"consome"| Contrato
    Banco -.->|"implementa"| Contrato
```
...
```mermaid
flowchart LR
    InterruptorDireto["Interruptor<br>acoplado a Ventilador"]:::warning
    VentiladorDireto["Ventilador"]:::warning
    Interruptor["Interruptor"]:::core
    Contrato["ILigavel"]:::data
    Ventilador["Ventilador"]:::component
    Lampada["Lampada"]:::component
    TV["Televisao"]:::component

    InterruptorDireto -->|"referencia diretamente"| VentiladorDireto
    Interruptor -->|"depende de"| Contrato
    Ventilador -.->|"implementa"| Contrato
    Lampada -.->|"implementa"| Contrato
    TV -.->|"implementa"| Contrato
```
