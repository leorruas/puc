...
## SOLID como estratégia para reduzir acoplamento e rigidez
...
### Intuição: modularidade em vez de uma ferramenta monolítica
...
### Separação de preocupações como base arquitetural
...
## Aplicação dos princípios: violação e refatoração
...
#### Motivo de mudança como ator responsável
...
#### Efeitos de misturar responsabilidades de atores diferentes
...
#### Estudo de caso: anúncio publicitário e detector gramatical

Um dos métodos mais práticos e ensinados por Uncle Bob para identificar violações do **SRP** é analisar a descrição em linguagem natural da responsabilidade da classe:

> **A Heurística Gramatical de Uncle Bob:**
> Se para descrever o que uma classe faz você precisa usar o conectivo aditivo **"E"** (ex.: *"Esta classe armazena os dados do anúncio **E** calcula as estatísticas de exibição"*), **o SRP está sendo violado**.

```mermaid
flowchart TD
    subgraph EstudoCasoAnuncio ["Estudo de Caso: A Decomposição de Responsabilidades do Anúncio"]
        direction TB

        subgraph ViolacaoSRP ["❌ Violação do SRP (2 Motivos de Mudança Misturados)"]
            direction TB
            AN["Classe Anuncio<br>• Armazena Informações do Informe Publicitário<br>• E Calcula Estatísticas de Redes Sociais"]
        end

        subgraph SolucaoSRP [" Refatoração com SRP: 2 Classes Coesas"]
            direction TB
            A1["Classe Anuncio<br>(Propriedades e Estado do Informe)"]
            A2["Classe AnalisadorCampanhaPublicitaria<br>(Cálculos Estatísticos de Alcance e ROI)"]
            A1 ~~~ A2
        end
...
#### Violação de SRP na classe `Anuncio`
A classe acumula **duas razões completamente distintas para ser alterada**:
1. **Primeiro motivo de mudança (Dados do Anúncio):** Mudanças solicitadas pela equipe de criação/conteúdo sobre os metadados do anúncio (título, texto, imagens, data de validade, formato de mídia).
2. **Segundo motivo de mudança (Análise Estatística da Campanha):** Mudanças solicitadas pela equipe de marketing analítico ou cientistas de dados sobre métricas de engajamento (taxa de cliques - CTR, custo por clique - CPC, impressões por rede social e algoritmos estatísticos).

A solução arquitetural correta é manter `Anuncio` como uma entidade de dados pura e criar uma classe de serviço especializada como `CalculadoraEstatisticasAnuncio` ou `AnalisadorCampanha`.
...
#### Relação entre OCP e acoplamento
...
#### OCP, herança e a formulação de Bertrand Meyer
...
#### Por que violações de LSP passam despercebidas
...
#### Relação "é um" versus compatibilidade comportamental
...
#### Limites da verificação estática para LSP
...
#### Design by Contract aplicado ao LSP
...
#### Contraexemplo do retângulo e do quadrado
...
#### Efeito de violações de ISP sobre outros princípios
...
#### Relação entre ISP, OCP e acoplamento
...
#### Dependência orientada para abstrações

Essa é uma das frases mais elegantes e profundas do SOLID, dividida em duas regras complementares:

```mermaid
flowchart TD
    subgraph AnatomiaDIPRegra2 ["Anatomia da Regra: Abstração vs. Detalhes"]
        direction TB

        subgraph RegraA ["1. 'Abstrações NÃO devem depender de detalhes'"]
            direction TB
            A_ERR["❌ Interface Poluída com Detalhes Concretos:<br>interface IRepositorioCliente {<br>   void SalvarComSqlConnection(SqlConnection conn);<br>   SqlDataReader BuscarPorQuerySql(string sqlQuery);<br>}"]
            A_COR[" Interface Pura e Neutra de Domínio:<br>interface IRepositorioCliente {<br>   void Salvar(Cliente c);<br>   Cliente? BuscarPorId(Guid id);<br>}"]
            A_ERR ~~~ A_COR
        end

        subgraph RegraB ["2. 'Detalhes DEVEM depender de abstrações'"]
            direction TB
            B_IMP["As tecnologias voláteis (SQL, Oracle, MongoDB, REST)<br>devem se curvar e implementar o contrato exigido pelo negócio."]
        end
...
#### Interfaces neutras em relação à tecnologia
* Uma interface (`IRepositorio`, `INotificador`) representa a **necessidade pura do negócio**, sem saber como será realizada no mundo físico.
* Se a sua interface contém tipos como `SqlDataReader`, `OracleTransaction`, `HttpRequestMessage` ou caminhos de arquivo no disco, a abstração foi **contaminada com detalhes de implementação**.
* **A regra:** A interface define **O QUE** deve ser feito (`Salvar`), nunca **COMO** a tecnologia externa fará (`SalvarComQuerySql`).

#### Detalhes concretos adaptados ao contrato
* **Analogia de Feynman:** A **tomada da parede** é a *Abstração* (define um padrão estável de 110V/220V e dois pinos). A *geladeira*, o *computador* e o *carregador de celular* são os *Detalhes concretos*.
* A tomada da parede **não foi desenhada sob medida para a sua geladeira da marca X**; foi a fábrica da geladeira que se adaptou e construiu o plugue para encaixar no padrão universal da tomada da parede.
* No código: O banco de dados (detalhe) é quem deve construir uma classe que se encaixe na interface definida pelo seu modelo de negócio (abstração).
...
#### Estudo de caso: interruptor e dispositivos

Imagine a modelagem de um sistema de automação predial com uma classe `Interruptor` e uma classe concreta `Ventilador`.

```mermaid
flowchart TD
    subgraph InterruptorEstudoCaso ["Estudo de Caso: Por que o Acoplamento Concreto quebra DIP e OCP Simultaneamente"]
        direction TB

        subgraph Incorreto ["❌ Modelagem Incorreta (Viola DIP e OCP)"]
            direction TB
            INT1["Interruptor<br>• Ligar()<br>• Desligar()"] -->|Referencia diretamente classe concreta| VENT1["Ventilador<br>• AcionarMotor()<br>• PararMotor()"]
            ALT["Para controlar uma Lâmpada ou TV,<br>é OBRIGATÓRIO MODIFICAR a classe Interruptor! 💥 (Viola OCP)"]
            INT1 ~~~ ALT
        end

        subgraph Correto [" Modelagem Correta com Inversão e Extensibilidade"]
            direction TB
            INT2["Interruptor (Controle)"] -->|Depende apenas da interface| ILIG["&lt;&lt;interface&gt;&gt;<br>ILigavel<br>• Ligar()<br>• Desligar()"]
            VENT2["Ventilador"] -.->|Implementa| ILIG
            LAMP["Lampada"] -.->|Implementa| ILIG
            TV["Televisao"] -.->|Implementa| ILIG
        end
...
#### Violação conjunta de DIP e OCP

1. **Violação do DIP (Princípio da Inversão de Dependência):**
   - O `Interruptor` (módulo de controle / alto nível) referencia diretamente uma **classe concreta** de dispositivo (`Ventilador`), em vez de depender de uma abstração.
   - O alto nível fica refém dos detalhes de implementação do motor do ventilador.

2. **Violação do OCP (Princípio do Aberto/Fechado):**
   - Como o `Interruptor` está amarrado ao tipo `Ventilador`, se amanhã quisermos usar o mesmo interruptor para ligar uma `Lampada`, um `ArCondicionado` ou uma `Televisao`, seremos **obrigados a abrir e alterar o código-fonte da classe `Interruptor`** (criando novos métodos ou blocos `switch/case`).
   - O interruptor não está *fechado para modificação*, tampouco *aberto para extensão*.

3. **A Solução Elegante:**
   - Criar o contrato abstrato `ILigavel` com os métodos `Ligar()` e `Desligar()`.
   - O `Interruptor` recebe qualquer `ILigavel` via injeção por construtor. Qualquer novo aparelho do mundo poderá ser controlado sem tocar em uma única linha do `Interruptor`.

---

## Matriz de revisão dos princípios SOLID
...
## SOLID em um sistema bancário integrado
...
* **Próximo artigo:** [[24. Padrões de projeto (design patterns) e o catálogo GoF]]
