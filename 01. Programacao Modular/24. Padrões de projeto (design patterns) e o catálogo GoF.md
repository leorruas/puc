...
# Padrões de projeto (design patterns): fundamentos, catálogo GoF e os três grandes grupos
...
## Padrões de projeto como soluções recorrentes de design
...
### Intuição: adaptar uma solução estrutural comprovada
...
## Da linguagem de padrões à Gang of Four
...
## Princípios de POO que sustentam os padrões GoF
...
## Classificação dos padrões GoF por propósito

O catálogo GoF classifica os 23 padrões em três grandes categorias, conforme o propósito arquitetural:

```mermaid
flowchart TD
    GoF["Catálogo de Padrões GoF<br>(23 Padrões Clássicos)"]:::core

    G1["Padrões Criacionais<br>(Instanciação e desacoplamento de new)"]:::component
    G2["Padrões Estruturais<br>(Composição de classes e objetos)"]:::component
    G3["Padrões Comportamentais<br>(Comunicação e distribuição de responsabilidades)"]:::component
...
### Escopo de classe e de objeto

O livro clássico do GoF organiza os 23 padrões em uma **matriz bidimensional**, cruzando o **Escopo** (se o mecanismo atua sobre classes ou sobre objetos) com o **Propósito** (Criacional, Estrutural ou Comportamental):

* **Escopo de Classe:** O padrão lida com os relacionamentos estáticos entre classes estabelecidos em **tempo de compilação** por meio de **herança** (reutilização caixa-branca / *white-box*).
* **Escopo de Objeto:** O padrão lida com as relações dinâmicas entre instâncias estabelecidas em **tempo de execução** por meio de **composição e delegação** (reutilização caixa-preta / *black-box*).

| Escopo | Criacional | Estrutural | Comportamental |
| :--- | :--- | :--- | :--- |
| **Classe** *(Herança)* | `Factory Method` | `Class Adapter` *(Adapter de Classe)* | `Interpreter`<br>`Template Method` |
| **Objeto** *(Composição)* | `Abstract Factory`<br>`Builder`<br>`Prototype`<br>`Singleton` | `Object Adapter` *(Adapter de Objeto)*<br>`Bridge`<br>`Composite`<br>`Decorator`<br>`Facade`<br>`Flyweight`<br>`Proxy` | `Chain of Responsibility`<br>`Command`<br>`Iterator`<br>`Mediator`<br>`Memento`<br>`Observer`<br>`State`<br>`Strategy`<br>`Visitor` |
...
## Combinação de `Singleton`, `Strategy` e `Observer`
...
## Síntese e critérios de escolha
