...
# Destrutores e finalizadores (desalocação de memória e liberação de recursos)
...
## Objetos inalcançáveis e coleta de lixo
...
### Ciclo de vida após a perda de referências
...
**Modelo mental da coleta de lixo.**
...
## Limpeza determinística com `IDisposable` e `using`
...
### Finalizadores, `Dispose` e `using` em C#
...
### Exemplo integrado de liberação de recursos
...
## Coleta de lixo, finalização e recursos não gerenciados
...
**Finalização e threads de execução.**
...
3. **O Perigo do Bloqueio de Thread (*Thread Blocking*):** Se você colocar um laço infinito, uma pausa pesada (`Thread.Sleep`) ou uma operação de rede lenta dentro de um destrutor `~Classe()`, você travará a *Finalizer Thread* inteira do .NET, impedindo que todos os outros destrutores da aplicação rodem! → [[Glossário de conceitos#Thread linha de execução Thread|Thread no Glossário]].
...
**Buffers e liberação de recursos.**
...
* **A caixa de saída dos correios:**
  ...
  - Em escritores como `StreamWriter`, `Dispose()` descarrega o buffer gerenciado. Não confie no finalizador para salvar dados: ele não garante esse trabalho. `Flush` também não equivale, em todas as APIs, à persistência física resistente a falta de energia. → [[Glossário de conceitos#Buffer e descarregamento de dados Buffer & Flush|Buffer no Glossário]].
...
### Destrutor e finalizador na terminologia de C#
...
**Por que finalizadores não servem para tarefas críticas.**
...
## Construtor, finalizador e `Dispose`: responsabilidades diferentes
...
### Comparação entre construção, finalização e descarte
