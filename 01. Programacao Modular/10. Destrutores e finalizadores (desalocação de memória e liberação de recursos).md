...
```mermaid
flowchart LR
    Alcancavel["Objeto alcançável"]:::component
    Inalcancavel["Objeto inalcançável"]:::warning
    Elegivel["Elegível para coleta"]:::data
    Finalizacao["Se houver finalizador<br>entra no processo de finalização"]:::component
    Recuperacao["Memória pode ser recuperada"]:::core

    Alcancavel --> Inalcancavel
    Inalcancavel --> Elegivel
    Elegivel --> Finalizacao
    Finalizacao --> Recuperacao
```
...
flowchart LR
    Using["using"]:::core
    Uso["Usa o recurso"]:::component
    Dispose["Saída do escopo<br>chama Dispose"]:::data
...
```mermaid
flowchart LR
    Criacao["Construtor<br>incrementa contador didático"]:::component
    Finalizacao["Finalizador<br>decrementa quando executado"]:::component
    Alerta["Não representa com precisão<br>quantos objetos estão vivos agora"]:::warning
...
```mermaid
flowchart LR
    Aplicacao["Threads da aplicação<br>alocam objetos"]:::external
    GC["Garbage Collector<br>atua conforme o modo do runtime"]:::core
    Fila["Objetos finalizáveis<br>podem ser enfileirados"]:::data
    Finalizer["Thread de finalização<br>executa finalizadores"]:::component
...
| Categoria | Exemplos | Quem cuida da memória / liberação |
| :--- | :--- | :--- |
| Objetos gerenciados comuns | `string`, listas, arrays, objetos C# | O GC recupera a memória quando ficam inalcançáveis |
| Objetos gerenciados que encapsulam recursos externos | `FileStream`, `SqlConnection`, sockets | O objeto é gerenciado, mas o recurso externo deve ser liberado pelo padrão de descarte apropriado |
| Recursos nativos diretos | Handles, ponteiros nativos, memória não gerenciada | Exigem uma estratégia explícita de liberação, normalmente encapsulada por tipos seguros |
...
```mermaid
flowchart LR
    App["Aplicação"]:::external
    Buffer["Buffer em memória"]:::data
    Destino["Arquivo / rede"]:::component
    Dispose["Dispose / flush apropriado"]:::core
    Pendente["Sem descarte, bytes podem<br>permanecer pendentes"]:::warning

    App -->|"escreve"| Buffer
    Dispose -->|"descarrega conforme a API"| Buffer
    Buffer --> Destino
    Buffer -.-> Pendente
```
...
| Conceito | C++ | C# |
| :--- | :--- | :--- |
| Destruição / finalização | Destrutores participam do ciclo de vida determinado pelas regras de duração do objeto | A sintaxe `~Classe()` declara um finalizador gerenciado |
| Momento | Pode ser determinístico conforme armazenamento e escopo | Não determinístico |
| Memória | O modelo inclui destruição explícita ou automática conforme o tipo de armazenamento | A memória gerenciada é recuperada pelo GC |
| Uso recomendado para recursos | RAII é central | `IDisposable`/`using` para descarte determinístico; finalizador é uma rede de segurança específica |
