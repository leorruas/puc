...
3. **O Perigo do Bloqueio de Thread (*Thread Blocking*):** Se você colocar um laço infinito, uma pausa pesada (`Thread.Sleep`) ou uma operação de rede lenta dentro de um destrutor `~Classe()`, você travará a *Finalizer Thread* inteira do .NET, impedindo que todos os outros destrutores da aplicação rodem! $\rightarrow$ [[Glossário de conceitos#21 Thread linha de execução Thread|Thread no Glossário]].
...
* **A caixa de saída dos Correios:**
  ...
  - O método `Dispose()` ou o destrutor executam o **despejo forçado (*Flush*)**, esvaziando a caixa e garantindo que cada último byte seja gravado no HD antes de trancar a porta. $\rightarrow$ [[Glossário de conceitos#20 Buffer e descarregamento de dados Buffer & Flush|Buffer no Glossário]].
