...
## 3. O que é um destrutor em C#? (`~NomeDaClasse`)

Em C#, o **destrutor** (chamado internamente de **Finalizador / *Finalizer***) é um método especial que possui o mesmo nome da classe precedido pelo caractere til (`~`):
...
public class ConexaoHardware 
{
    ...
    ~ConexaoHardware() 
    {
        ...
        Console.WriteLine("Destrutor acionado: liberando portas físicas...");
    }
}
```

### Características e regras dos destrutores em C#:
1. **Sem modificadores nem parâmetros:** Não aceita `public`, `private`, `static` e **nunca recebe parâmetros**.
2. **Execução Não Determinística (*Non-Deterministic*):** Você **não tem como prever exatamente o milissegundo** em que o destrutor será executado. Ele só roda quando o Garbage Collector decidir coletar a memória.
3. **Não pode ser chamado manualmente:** É proibido escrever `objeto.~Classe()` ou forçar sua chamada direta.
4. **Custo de Performance:** Objetos com destrutores sobrevivem por mais tempo no *Heap*, pois são movidos para uma fila especial chamada **Fila de Finalização (*Finalization Queue*)**, exigindo pelo menos duas passadas do GC para serem totalmente reciclados.
