...
### O que são classes e membros selados? (A intuição da técnica de Feynman)
...
flowchart TD
    subgraph SelagemAnalogia ["A analogia do lacre de segurança inviolável"]
        ...
        Selada["3. Classe Selada (sealed): Documento Reconhecido em Cartório e Lacrado<br>(proíbe herdar da classe; não congela seus objetos)"]
...
### Membros selados (`sealed override`)
...
### Reflexão conceitual: Uma classe abstrata pode ser selada? E métodos em classes abstratas?

Esta é uma questão clássica para testar a compreensão dos fundamentos da POO:

```mermaid
flowchart TD
    subgraph ReflexaoAbstractSealed ["Reflexão: Incompatibilidade de Classes vs. Compatibilidade de Métodos"]
        direction TB

        subgraph Incompativel ["1. abstract class + sealed class = Paradoxo Lógico (ERRO CS0418)"]
            direction TB
            A1["abstract: 'proibido instanciar, OBRIGATÓRIO ter filhos que herdem'"]
            A2["sealed: 'proibido ter filhos que herdem'"]
            A1 <-->|Contradição direta| A2
        end

        subgraph Compativel ["2. abstract class com método sealed override = Válido e Comum"]
            direction TB
            B1["A classe intermediária é abstrata (não pode dar new)."]
            B2["Ela herda um método virtual do avô, sobrescreve e aplica 'sealed override'."]
            B3["Suas subclasses concretas herdam o método travado e não podem mais alterá-lo."]
            B1 --> B2 --> B3
        end

        Incompativel ~~~ Compativel
    end
```

#### Uma classe abstrata pode ser selada (`abstract sealed class`)?
...
#### Uma classe abstrata pode ter métodos selados (`sealed override`)?
* **SIM, com certeza!**
* **Como funciona?**
...
### Por que selar classes e membros? (A tríade: segurança, performance e type safety)
...
### O custo do polimorfismo dinâmico vs. a otimização de performance do `sealed`
...
flowchart TD
    subgraph MecanicaExecucao ["Anatomia de execução: Polimórfico Dinâmico vs. Selado Estático"]
        ...
        subgraph FluxoSelado ["2. Chamada Devirtualizada / Estática (Com Sealed)"]
            ...
            S2["2. Pode evitar consulta indireta quando o tipo é conhecido: o endereço é estático e cravado na compilação."]
...
### Matriz comparativa dos modificadores de herança
...
### Exemplo completo e integrado em C# (Sistema financeiro de pagamentos e segurança)
...
### Classe privada vs. classe selada (*Private vs. Sealed*)
...
```mermaid
flowchart TD
    subgraph ComparativoPrivateSealed ["Diferença Estrutural: Classe Privada vs. Classe Selada"]
        direction TB

        subgraph Privada ["1. Classe Privada (private class - Aninhada)"]
            direction TB
            PR1["Localização: só pode existir aninhada DENTRO de outra classe."]
            PR2["Escopo: NUNCA pode ser declarada solta em um namespace."]
            PR3["Acesso: seus membros só são visíveis dentro da classe hospedeira."]
            PR1 --> PR2 --> PR3
        end

        subgraph Selada ["2. Classe Selada (sealed class - De Topo)"]
            direction TB
            SE1["Localização: pode ser declarada livremente em qualquer namespace."]
            SE2["Instanciação: pode ser pública (public) e instanciada em qualquer lugar do sistema."]
            SE3["Herança: É a herança que está bloqueada, NÃO o uso de seus objetos."]
            SE1 --> SE2 --> SE3
        end
...
### Quando usar e quando não usar `sealed`? (Guia de boas práticas)
