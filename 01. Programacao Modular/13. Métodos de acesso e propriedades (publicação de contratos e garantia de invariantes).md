...
### a. Propriedades autoimplementadas (*Auto-Implemented Properties* e eliminação de *Boilerplate*)
...
### d. Propriedades calculadas (*Computed / Expression-Bodied Properties* e o Paradigma Funcional)
...
* **O operador de expressão lambda (`=>`):** A sintaxe com seta (veja [[Glossário de conceitos#37 Expressão lambda e membros com corpo de expressão Lambda Expressions Expression Bodied Members|Expressão lambda no glossário]]) substitui a cerimônia de abrir chaves `{ get { return ...; } }` por uma declaração matemática direta.
* **A conexão com o paradigma funcional:** O C# moderno é multiparadigma (veja [[Glossário de conceitos#38 Paradigma funcional Functional Programming Paradigm|Paradigma funcional no glossário]]). Uma propriedade calculada funciona como uma **função matemática pura**: ela não produz efeitos colaterais (*side effects*), não altera nenhuma variável na memória e sempre devolve o mesmo resultado exato para os mesmos valores de entrada.
...
public class Retangulo 
{
    ...
    // Propriedades calculadas funcionais via expressão lambda (=>):
...
#### A facilidade do `get` com expressão lambda (`get => id;`):

O C# permite usar a expressão lambda tanto na propriedade inteira quanto **apenas dentro do bloco `get` ou `set` individual** (*Expression-Bodied Accessors*):
...
* **Comparativo com a sintaxe clássica:**
  ...
  - **Com lambda (1 linha):** `public int Id { get => _id; }` ou `public int Id => _id;`
