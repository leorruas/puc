...
#### A regra fundamental de assinatura e tipos:
1. **O método `Get` SEMPRE retorna o tipo exato do atributo:**
   - Como sua função é consultar uma informação, ele é uma **Função com retorno** (`public decimal ObterPreco()`, `public string ObterNome()`, `public int ObterIdade()`).
   - O tipo de retorno depende estritamente do tipo do atributo que está sendo lido.
2. **O método `Set` é SEMPRE declarado como `void` (Procedimento):**
   - A finalidade exclusiva do `Set` é **executar uma ação / comando de mutação** no estado interno (`public void DefinirPreco(decimal novoPreco)`).
...
public class Produto 
{
    ...
    // Método Getter: Função com retorno do tipo 'decimal' (apenas leitura)
    public decimal ObterPreco() 
    {
        return _preco;
    }

    // Método Setter: Procedimento 'void' que executa o comando com validação
...
* **Uso pelo cliente:** O código cliente utiliza sintaxe limpa de atribuição (`p.Preco = 150.00m;`), mas o compilador executa a rotina `set` com toda a validação!
...
* **Vantagem de engenharia:** Permite a sintaxe fluida de inicializadores de objeto preservando 100% da imutabilidade e proteção contra mutação indevida.
...
1. **Evite *Getters* e *Setters* que apenas expõem tudo sem critério:**
...
2. **Prefira métodos de domínio a *Setters* genéricos:**
   - Em vez de fazer `conta.Saldo = conta.Saldo + 100;`, forneça o método de negócio `conta.Depositar(100);`.
3. **Propriedades calculadas não devem conter processamento pesado:**
