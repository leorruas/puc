...
```mermaid
flowchart LR
    Cliente["Código cliente"]:::external
    Propriedade["Propriedade Saldo"]:::core
    Get["get"]:::component
    Set["set com validação"]:::component
    Campo["_saldo"]:::data

    Cliente -->|"lê"| Propriedade
    Cliente -->|"solicita alteração"| Propriedade
    Propriedade --> Get
    Propriedade --> Set
    Get --> Campo
    Set -->|"se válido"| Campo
...
```mermaid
flowchart TD
    Q1{"O cliente externo precisa<br>observar esse dado?"}:::core
    Oculto["Mantenha oculto"]:::component
    Q2{"O cliente deve alterar<br>diretamente o valor?"}:::core
    Leitura["Exponha leitura"]:::data
    Q3{"A alteração é uma simples atribuição<br>com validação local?"}:::core
    Propriedade["Propriedade com setter validado"]:::component
    Dominio["Método de domínio<br>expressa a operação"]:::component
...
| Elemento | Convenção comum em C# | Exemplo |
| :--- | :--- | :--- |
| Classes, structs, métodos e propriedades | PascalCase | `ContaBancaria`, `Depositar`, `Saldo` |
| Interfaces | PascalCase com prefixo `I` por convenção .NET | `IValidavel` |
| Parâmetros e variáveis locais | camelCase | `novoPreco`, `totalTentativas` |
| Campos privados | `_` + camelCase em muitas bases .NET | `_saldo`, `_precoUnitario` |
...
```mermaid
flowchart LR
    CampoPublico["Campo público mutável"]:::warning
    Propriedade["Propriedade / método<br>mediando o acesso"]:::core
    Estado["Estado interno"]:::data

    CampoPublico -->|"expõe diretamente"| Estado
    Propriedade -->|"valida e controla"| Estado
```
