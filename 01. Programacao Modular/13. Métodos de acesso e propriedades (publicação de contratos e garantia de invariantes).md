...
# Métodos de acesso e propriedades (publicação de contratos e garantia de invariantes)
...
O cliente precisa consultar o saldo, mas isso não implica poder substituí-lo livremente. Uma propriedade publica uma informação por meio de acessores; um método de domínio publica uma operação. Escolher esse contrato vem antes de escrever `get` e `set`.

### Por que ocultar atributos e publicar métodos?
...
* **Definições formais no glossário:**
  * [[Glossário de conceitos#Métodos de acesso ou getters Accessors Getters|Métodos de acesso ou getters (*Accessors / Getters*)]] — Operações públicas de somente leitura.
  * [[Glossário de conceitos#Métodos modificadores ou setters Mutators Setters|Métodos modificadores ou setters (*Mutators / Setters*)]] — Operações públicas de escrita com validação de invariantes.
  * [[Glossário de conceitos#Propriedade Property|Propriedade (*Property*)]] — Membro nativo que combina sintaxe de campo com segurança de métodos.
...
### Quando faz sentido que um atributo tenha método de acesso (e quando não faz)?
...
##### Quando faz sentido ter apenas o `Get` (somente leitura):
...
##### Quando faz sentido ter `Get` e `Set` (com validação rigorosa):
...
##### Quando não faz sentido ter `Set` público:
...
##### Quando não faz sentido ter `Get` nem `Set` (segredos ocultos):
...
> [!IMPORTANT]
> **Reflexão de design: por que o atributo `Saldo` não deve ter `SetSaldo`?**
> Nem todo atributo de uma classe precisa ter métodos *Getters* e *Setters*. Imagine o atributo `_saldo` de uma classe `ContaCorrente`: realmente faria sentido alterarmos o saldo diretamente através de um método `SetSaldo(5000)`?
>
> A resposta é um categórico **não**. Em sistemas bancários reais, um saldo nunca muda magicamente por mera atribuição de um valor arbitrário. O saldo só deve ser alterado como consequência de operações legítimas de negócio, como **saques com conferência de fundos** (`conta.Sacar(valor)`) ou **depósitos com validação de cédulas** (`conta.Depositar(valor)`).
>
> **A regra de ouro:** Sempre que criar uma classe, antes de declarar *Getters* e *Setters* automaticamente para todos os campos, **pense rigorosamente na lógica do negócio e no significado da entidade no mundo real**!

---

## Mecânica em C#

Compare métodos convencionais, propriedades completas e autoimplementadas. Depois diferencie `get`, `private set`, `init` e propriedades calculadas. O exemplo de estoque reúne validação no construtor, preço validado e operações de entrada e saída.

### As duas abordagens de implementação no C#

Na linguagem C#, podemos implementar o controle de acesso de duas formas:

#### Abordagem clássica por métodos convencionais (`Get` e `Set`)
...
1. **Um getter simples consulta o objeto sem argumentos explícitos:**
   ...
   - O tipo de retorno expressa a informação publicada; pode coincidir com o campo ou expor uma projeção mais adequada (`public decimal ObterPreco()`).
2. **No padrão didático de setter, o método retorna `void` e recebe um parâmetro formal:**
   ...
   - **Por que o `Set` deve ser `void`? (Princípio CQS - *Command-Query Separation* de Bertrand Meyer):**
     - Uma sub-rotina deve fazer uma pergunta (*Query* → retornar dados sem alterar estado) **ou** executar um comando (*Command* → alterar estado sem retornar dados).
     ...
     - Quando o `Set` não consegue concluir sua tarefa devido a um valor inválido, a API pode lançar uma exceção ou adotar um método como `TryDefinirPreco` que retorne `bool`. CQS orienta a separação de responsabilidades, mas não proíbe todo comando com retorno; o contrato deve tornar o resultado e os efeitos claros.
...
#### Abordagem idiomática do C#: propriedades completas (*full properties*)
...
### Tipos e variações de propriedades no C#

O ecossistema C# oferece suporte a diferentes níveis de sofisticação para atender a requisitos variados de design:

#### Propriedades autoimplementadas (*auto-implemented properties* e eliminação de *boilerplate*)

Em linguagens como Java ou C++ tradicional, encapsular 5 atributos exige escrever mais de 40 linhas de **código *boilerplate*** (código burocrático e repetitivo que não agrega lógica de negócio — veja [[Glossário de conceitos#Código boilerplate Boilerplate Code|Código boilerplate no glossário]]).

No C#, quando um atributo não exige validação complexa imediata, usamos **propriedades autoimplementadas**:
* O compilador cria e gerencia o atributo privado de suporte (*backing field*) nos bastidores de forma invisível.
* Você reduz 10 linhas de cerimônia burocrática para **1 única linha limpa e elegante**:

```csharp
public class Cliente
{
    // O compilador cria internamente o backing field privado anônimo (Zero boilerplate!)
...
#### Propriedades somente leitura (*read-only properties*)
...
#### O acessador `init` (imutabilidade pós-inicialização - C# 9+)
...
* **O que faz:** O `init` permite que a propriedade seja atribuída **apenas durante a construção do objeto** (seja no construtor ou via *Object Initializer* `{ Propriedade = valor }`). Após a conclusão da instanciação, a atribuição posterior por código comum é bloqueada. Isso não torna profundamente imutável um objeto retornado pela propriedade, nem obriga o chamador a fornecer um valor válido.
* **Vantagem de engenharia:** Permite a sintaxe fluida e legível de inicializadores de objeto sem poluir a classe com dezenas de sobrecargas de construtores, restringindo a fase de atribuição. Validação e obrigatoriedade de preenchimento são decisões adicionais.
...
#### Propriedades calculadas (*computed / expression-bodied properties* e o paradigma funcional)

Propriedades calculadas não possuem atributo de suporte na memória (*backing field*); seu valor é computado dinamicamente no exato instante em que o cliente faz a leitura:

* **O corpo de expressão (`=>`):** A sintaxe com seta (veja [[Glossário de conceitos#Expressão lambda e membros com corpo de expressão Lambda Expressions Expression Bodied Members|Expressão lambda no glossário]]) substitui a cerimônia de abrir chaves `{ get { return ...; } }` por uma declaração matemática direta.
* **A conexão com o paradigma funcional:** O C# moderno é multiparadigma (veja [[Glossário de conceitos#Paradigma funcional Functional Programming Paradigm|Paradigma funcional no glossário]]). Uma propriedade calculada **pode ser escrita como um cálculo sem efeitos colaterais**, como a área do retângulo abaixo. C# não impõe pureza: o getter pode consultar relógio, modificar estado ou chamar código com efeitos. A seta aqui introduz um membro com corpo de expressão; não cria, por si só, uma expressão lambda.
...
public class Retangulo
{
    ...
    // Propriedades calculadas com corpo de expressão (=>):
    // Lê-se: 'A propriedade Area resulta na expressão Largura * Altura'
    public double Area => Largura * Altura;
    public double Perimetro => 2 * (Largura + Altura);
}
```

##### A forma concisa do `get` com corpo de expressão (`get => id;`):

O C# permite usar corpo de expressão tanto na propriedade inteira quanto **apenas dentro do bloco `get` ou `set` individual** (*Expression-Bodied Accessors*):
...
public class Usuario
{
    ...
    // Forma ultra concisa para encapsular um campo privado de leitura:
    public int Id { get => _id; }
}
```

* **Comparativo com a sintaxe clássica:**
  - **Clássico (4 linhas):** `public int Id { get { return _id; } }`
  - **Com corpo de expressão (1 linha):** `public int Id { get => _id; }` ou `public int Id => _id;`
* **Vantagem:** Reduz a poluição visual do arquivo sem perder a segurança do encapsulamento, combinando a leitura de um *backing field* privado com extrema elegância e concisão.

---

#### Propriedades com visibilidade assimétrica
...
#### Métodos e propriedades de classe com `static`
...
##### Quando publicar métodos e propriedades como `static`?
1. **Manipulação de estado global compartilhado:** Se você possui um atributo estático privado (como um contador total de contas criadas ou uma taxa Selic global), declarar o acesso como `static` permite usá-lo sem uma instância. Métodos de instância também podem acessar campos estáticos; isso é permitido pela linguagem:
...
2. **Métodos utilitários e funções matemáticas puras:** Métodos que operam exclusivamente com base nos parâmetros que recebem (sem depender de nenhum atributo interno de um objeto) podem ser bons candidatos a `static` (ex.: `Math.Sqrt(x)`). `Convert.ToInt32(s)` pode lançar exceções, e `DateTime.UtcNow` é uma propriedade dependente do relógio: ser estático não significa ser puro. Contratos de interface também podem justificar métodos de instância sem estado próprio.
...
#### Validação rigorosa no `set` protegendo invariante
...
#### Propriedade com `private set` (mutação restrita a métodos de negócio)
...
namespace ProgramacaoModular.MetodosDeAcessoEPropriedades
{
    ...
    public class Produto
    {
        ...
        // 2. Propriedade autoimplementada: leitura pública e alteração privada
        ...
        public void AdicionarEstoque(int quantidade)
        {
            ...
            QuantidadeEmEstoque = checked(QuantidadeEmEstoque + quantidade);
        }
...
As convenções de nomes tornam a leitura consistente; não são exigências sintáticas do compilador. Também é preciso distinguir restrição de escrita, imutabilidade do objeto retornado e efeitos do getter.

### Permissão de escrita não é imutabilidade profunda

| Forma | Quem pode atribuir? | Limite da garantia |
| :--- | :--- | :--- |
| `{ get; }` autoimplementada | Inicializador da propriedade e construtor do tipo | O objeto retornado ainda pode ser mutável. |
| `{ get; private set; }` | Código com acesso ao setter privado | A própria classe pode alterar o valor após construir. |
| `{ get; init; }` | Contextos de inicialização permitidos | Não exige preenchimento nem valida o valor automaticamente. |
| `=> expressão` | Não há setter nessa declaração | O resultado pode variar e o getter pode ter efeitos. |

Uma coleção devolvida por getter pode expor o estado interno. Mesmo uma interface de leitura não torna automaticamente imutáveis os elementos; avalie cópia, visão de leitura e objetos imutáveis conforme o contrato. `init` não substitui validação de `Host` ou intervalo de `Porta` no exemplo de configuração.

Getters sem argumentos e setters `void` com um argumento são convenções para acesso simples. Métodos de consulta podem receber filtros; o retorno publicado não precisa revelar o tipo do campo. Propriedades calculadas não precisam de campo próprio, mas podem consultar outros campos. Corpos de expressão usam `=>` sem serem lambdas.

Centralizar validação em uma propriedade não exige tornar seu setter público. Evite chamar acessores virtuais durante a construção: a implementação derivada pode executar antes de estar pronta. Veja o comparativo de estouro em [[00. Sintaxe Multilinguagem/09. Tratamento de exceções e erros (try, catch, finally, throw)]]. No estoque integrado, `Nome` tem escrita privada, validada na construção, e a soma de quantidades usa `checked` para rejeitar estouro do inteiro.

### Convenções de nomenclatura no C#: `PascalCase` vs. `camelCase` vs. `_underline`

Para garantir legibilidade instantânea e consistência em projetos profissionais, as convenções de C# recomendam estilos de capitalização para cada tipo de identificador (consulte também [[Glossário de conceitos#Convenções de nomenclatura Naming Conventions camelCase e PascalCase|Convenções de nomenclatura no glossário]]):
...
### Matriz comparativa: campos vs. métodos vs. propriedades

| Característica | Campo público (`public int X;`) | Métodos `GetX()` / `SetX()` | Propriedades C# (`public int X { get; set; }`) |
| :--- | :--- | :--- | :--- |
| **Proteção de invariantes** | Não intercepta a atribuição | Depende da validação implementada | Depende do acessor; `{ get; set; }` não valida por si só |
...
| **Padrão na indústria C#** | Permitido; geralmente evitado em APIs de domínio | Adequado a operações e contratos explícitos | Idiomático para dados publicados com sintaxe de acesso |

---

## Conexões

**Continue a leitura:** [[08. Construtores (inicialização, sobrecarga e garantia de invariantes)|Artigo 08]] · [[09. Atributos estáticos e propriedades (compartilhamento de estado e encapsulamento)|Artigo 09]] · [[11. Princípio da ocultação da informação (information hiding e encapsulamento)|Artigo 11]] · [[12. Modificadores de acesso (visibilidade e níveis de proteção no encapsulamento)|Artigo 12]].

A analogia bancária retoma a fronteira de acesso. Os artigos 08, 09, 11 e 12 conectam inicialização, estado compartilhado, decisões ocultas e visibilidade ao contrato publicado aqui.

### A intuição do mundo real (técnica de Feynman): o caixa bancário blindado
...
2. **A janela de consulta do saldo ([[Glossário de conceitos#Métodos de acesso ou getters Accessors Getters|Getter / get]]):** Você pergunta ao caixa: *"Qual é o meu saldo?"*. O caixa olha no sistema e informa o valor. Você leu a informação sem tocar nas cédulas.
3. **A gaveta de depósito com validação ([[Glossário de conceitos#Métodos modificadores ou setters Mutators Setters|Setter / set]]):** Você entrega uma nota para depósito. O caixa passa a cédula no leitor de notas falsas e confere se o valor é positivo. Se for válido, ele guarda no cofre; se for nota falsa ou valor negativo, ele rejeita a transação imediatamente com um erro (*throw exception*).

---

### Boas práticas de projeto com métodos de acesso

1. **Aplique rigorosamente o Princípio DRY (*Don't Repeat Yourself*):**
   - Formulado por Andy Hunt e Dave Thomas (veja [[Glossário de conceitos#Princípio DRY Don t Repeat Yourself|Princípio DRY no glossário]]), o princípio dita que cada regra deve existir em um **único lugar definitivo**.
   - **Exemplo com propriedades:** Em vez de repetir a checagem `if (preco <= 0)` dentro do construtor e dentro de 3 métodos diferentes, concentre a regra **exclusivamente dentro do `set` da propriedade** e faça o construtor delegar a atribuição para a propriedade:
     ...
     ```
   - **Quando o DRY não se aplica? (atenção às abstrações prematuras):**
     - **Duplicação acidental:** Se duas classes de módulos diferentes têm um campo com o mesmo nome e regra por mera coincidência momentânea, não crie uma herança artificial nem force um acoplamento precoce.
     - *“A duplicação é muito mais barata do que a abstração errada”* (Sandi Metz). Só unifique quando o conceito de negócio for comprovadamente o mesmo.
...
4. **Propriedades calculadas não devem conter processamento pesado:**
   - Como propriedades são lidas com a sintaxe de variáveis (`p.ValorTotal`), os clientes esperam uma leitura previsível e de baixo custo; nem toda propriedade precisa ter complexidade $O(1)$. Se uma operação envolve ler arquivos do disco ou fazer consultas em banco de dados, declare como um **método explícito** (ex.: `p.CalcularTotalDoBanco()`).
...
* MICROSOFT. [Propriedades e acessores](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/properties). Microsoft Learn. Acesso em: 8 set. 2026.

### Artigos relacionados e navegação

* **Artigo anterior:** [[12. Modificadores de acesso (visibilidade e níveis de proteção no encapsulamento)]]
* **Próximo artigo:** [[14. Namespaces e partial classes (espaços de nomes e modularização em larga escala)]]
