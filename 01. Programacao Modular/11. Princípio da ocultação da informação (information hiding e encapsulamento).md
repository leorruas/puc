...
flowchart LR
    ...
    Interface["Interface pública estável"]:::core
    Estrutura["Estrutura de dados"]:::data
    Algoritmo["Algoritmo interno"]:::component
    Persistencia["Persistência / integração"]:::external

    Cliente -->|"depende de"| Interface
    Interface -.->|"oculta"| Estrutura
    Interface -.->|"oculta"| Algoritmo
    Interface -.->|"oculta"| Persistencia
```
...
| Decisão de projeto que pode ficar oculta | Exemplo de mudança interna |
| :--- | :--- |
| Estrutura de dados | Trocar lista por dicionário |
| Algoritmo | Alterar estratégia de cálculo ou ordenação |
| Persistência | Trocar arquivo, banco ou formato |
| Integração | Trocar driver, protocolo ou fornecedor |
...
| Princípio modular | Ideia central |
| :--- | :--- |
| Caixa-preta | O cliente conhece o contrato, não os detalhes |
| Independência funcional | Cada módulo possui responsabilidade própria |
| Alta coesão | Elementos internos trabalham para objetivos relacionados |
| Baixo acoplamento | Módulos conhecem o mínimo necessário uns dos outros |
...
```mermaid
flowchart TD
    Ocultacao["Ocultação da informação<br>proteger decisões que podem mudar"]:::core
    Encapsulamento["Encapsulamento<br>fronteira para estado e operações"]:::component
    Polimorfismo["Polimorfismo<br>cliente depende de contratos"]:::component
    Heranca["Herança<br>reuso e especialização sob regras de acesso"]:::component

    Ocultacao -->|"é implementada por"| Encapsulamento
    Ocultacao -->|"é favorecida por"| Polimorfismo
    Ocultacao -->|"pode coexistir com"| Heranca
```
