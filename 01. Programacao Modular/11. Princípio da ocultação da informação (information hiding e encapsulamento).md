...
4. **[[Glossário de conceitos#24 Polimorfismo Polymorphism|Acoplamento (*coupling*)]]:** É a **medida da interconexão e dependência** entre os elementos de software. Essa dependência pode ser medida pela quantidade e complexidade de conexões entre os módulos de um sistema. A situação ideal em engenharia é que um sistema possua **baixo acoplamento** $\rightarrow$ [[04. Programação orientada a objetos e acoplamento#5. O problema do acoplamento vs. coesão em sistemas modulares|Acoplamento em POO]].
...
Demonstração isolada de como esconder a estrutura interna permite refatorar a classe inteira sem alterar uma única linha do código cliente (observe o uso de `throw` para interromper a execução caso dados inválidos sejam fornecidos $\rightarrow$ veja a explicação completa no [[00. Sintaxe Multilinguagem/09. Tratamento de exceções e erros (try, catch, finally, throw)|Guia de sintaxe de tratamento de exceções (throw)]]):
