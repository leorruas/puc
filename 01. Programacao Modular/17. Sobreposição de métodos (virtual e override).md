...
No C#, métodos são **não-virtuais por padrão** (resolvidos estaticamente em tempo de compilação). Para habilitar o comportamento polimórfico dinâmico, duas palavras-chave trabalham em conjunto:
...
1. **`virtual` (Na classe base):**
   - Declara uma implementação padrão na superclasse e **concede permissão explícita** para que qualquer classe filha substitua esse comportamento.
2. **`override` (Na classe filha):**
   - Confirma a redefinição daquele método específico. O compilador valida se a assinatura (nome, tipos e parâmetros) coincide exatamente com o método `virtual` ancestral.
