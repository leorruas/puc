...
### O que é polimorfismo? (A intuição da técnica de Feynman)
...
### O modelo mental essencial: tipo declarado vs. tipo concreto (a referência polimórfica)
...
### A anatomia do polimorfismo de inclusão: `virtual` vs. `override`
...
### `override` vs. `new`: sobrescrita polimórfica vs. ocultação de membro
...
### Campos vs. métodos: por que campos NÃO participam do polimorfismo?
...
### A cláusula `base.Metodo()` (reutilização da lógica ancestral)
...
### A taxonomia formal: quais são os 4 tipos de polimorfismo?
...
#### Substituibilidade semântica ($S <: T$ e o Princípio de Liskov):
...
#### [[Glossário de conceitos#Vinculação tardia e despacho dinâmico (Late Binding & Dynamic Dispatch)|Despacho dinâmico (*late binding*)]] e a mecânica da *vtable*:
...
#### Eliminação total de cadeias de `if/else` e `switch` frágeis:
...
#### Analogia intuitiva de Feynman (A tomada universal e os eletrodomésticos):
...
### O caso especial de `ToString()` (a sobreposição universal de `System.Object`)
...
### Exemplo completo integrado e compilável (C#)
...
### Boas práticas no design de métodos sobrepostos
