...
* No **polimorfismo de inclusão**, o compilador não sabe qual método executará. A decisão é **postergada para o momento exato em que o programa está rodando**. A CPU consulta o [[Glossário de conceitos#60. Ponteiro (Pointer / vptr / Memory Pointer)|ponteiro *vptr*]] do objeto real alocado no *Heap*, abre sua [[Glossário de conceitos#54. Tabela de métodos virtuais (Virtual Method Table - vtable)|tabela de métodos virtuais (*vtable*)]] e salta para a implementação especializada correspondente.
...
### E as interfaces? O polimorfismo puro baseado em contratos (`can-do`)
...
```mermaid
flowchart TD
    subgraph PolimorfismoInterface ["Polimorfismo Baseado em Interfaces (Contratos Puros)"]
        direction TB

        Contrato["Interface Contratual: IImprimivel<br>(Exige o método: void Imprimir())"]

        subgraph ClassesDispares ["Classes Concretas de Domínios Totalmente Diferentes"]
            direction TB
            Relatorio["RelatorioPDF<br>(Domínio Financeiro)"]
            Nota["NotaFiscal<br>(Domínio Fiscal)"]
            Foto["FotoSatelite<br>(Domínio Multimídia)"]
        end

        Contrato -.->|Implementa| Relatorio
        Contrato -.->|Implementa| Nota
        Contrato -.->|Implementa| Foto

        Motor["Motor de Impressão:<br>void ImprimirTudo(List de IImprimivel itens)"]
        
        Motor -->|Despacho dinâmico| Contrato
    end
```

#### Por que o polimorfismo por interface é indispensável na engenharia de software?
...
flowchart TD
    subgraph DeiticoAnalogia ["Paralelo: linguística vs. computação polimórfica"]
        ...
        subgraph Linguistica ["1. Linguística: pronome dêitico"]
            direction TB
...
A variável `a` na *Stack* guarda internamente um **[[Glossário de conceitos#60. Ponteiro (Pointer / vptr / Memory Pointer)|ponteiro de memória]]** (endereço hexadecimal como `0x7F10`) que localiza a instância do objeto no *Heap*.
