...
### A evolução dos modelos mentais: de TADs isolados à hierarquia polimórfica

É natural sentir um impacto intelectual ao conectar essas três etapas do aprendizado em Programação Modular. O que estamos presenciando é a **evolução arquitetural da abstração de dados**:

```mermaid
flowchart TD
    subgraph EvolucaoModelosMentais ["A evolução cognitiva da abstração de software"]
        direction TB

        subgraph Fase1 ["1. TADs (Tipos Abstratos de Dados)"]
            direction TB
            TAD1["Abstração isolada: Encapsula dados + operações em uma cápsula fechada."]
            TAD2["Modelo Mental: Ilhas autossuficientes no oceano da memória (Conta, Data, Fila)."]
            TAD1 --> TAD2
        end

        subgraph Fase2 ["2. Herança (Taxonomia e Reúso)"]
            direction TB
            HER1["Relação estrutural: Cria linhagens genealógicas e compartilha estado/código comum."]
            HER2["Modelo Mental: Árvore genealógica (Conta &rarr; ContaPoupanca, ContaCorrente)."]
            HER1 --> HER2
        end

        subgraph Fase3 ["3. Polimorfismo (Comportamento Dinâmico)"]
            direction TB
            POL1["Desacoplamento comportamental: O cliente conversa com contratos abstratos ancestrais."]
            POL2["Modelo Mental: O controle universal onde cada subtipo reage com sua própria essência."]
            POL1 --> POL2
        end

        Fase1 -->|Conecta tipos em hierarquia| Fase2
        Fase2 -->|Dá vida dinâmica às especializações| Fase3
    end
```

#### Como cada camada expande o seu poder de abstração:

1. **O Tipo Abstrato de Dados (TAD - [[03. Tipos abstratos de dados|Artigo 03]]):**
   - **O que resolveu:** Resolveu o caos das variáveis globais e registros soltos. Você criou um molde com estado (`private`) e operações (`public`), garantindo invariantes e regras de negócio blindadas dentro de um único domínio.
   - **A limitação:** Cada TAD era uma ilha isolada. Uma `ContaCorrente` e uma `ContaPoupanca` eram dois TADs distintos, forçando você a duplicar código ou criar funções separadas para cada um.
2. **A Herança de Classes ([[15. Herança (generalização, especialização e extensibilidade modular)|Artigo 15]]):**
   - **O que resolveu:** Conectou esses TADs isolados em uma taxonomia coerente (*is-a*). O TAD `ContaCorrente` agora herda automaticamente os atributos (número, titular, saldo) e métodos da superclasse `Conta`, eliminando duplicações estruturais.
   - **A limitação:** Embora compartilhassem a estrutura, se a `ContaCorrente` precisasse cobrar uma taxa de saque diferente da `ContaPoupanca`, uma referência ancestral não conseguia invocar a versão correta sem recorrer a gambiarras e verificações manuais de tipo.
3. **O Polimorfismo com `virtual` e `override` (Este Artigo 17):**
   - **O salto quântico:** Fecha o ciclo da orientação a objetos. Você não precisa mais escrever código acoplado a subtipos concretos. O sistema passa a operar em nível de **protocolos e contratos abstratos** (`List<Conta>`). Ao invocar `conta.Sacar()`, a máquina executa o comportamento sob medida de cada subtipo no *runtime*.
   - **Em resumo:** *TADs criam as peças.* *Herança organiza as peças em famílias.* *Polimorfismo faz a orquestra inteira tocar em harmonia sem que o maestro precise conhecer o instrumento de cada músico individualmente.*
...
O **polimorfismo de inclusão** é a materialização do **polimorfismo dinâmico** na Orientação a Objetos pura:
1. **Substituibilidade semântica ($S <: T$):** Qualquer objeto de um subtipo $S$ pode ser manipulado com segurança em qualquer lugar onde o supertipo $T$ é esperado (veja [[15. Herança (generalização, especialização e extensibilidade modular)#3. Subtipagem vs. herança: o que é a relação S <: T?|Artigo 15]] e [[Glossário de conceitos#44. Subtipagem (Subtyping / Subtype Polymorphism)|Glossário 44]]).
2. **[[Glossário de conceitos#51. Vinculação tardia e despacho dinâmico (Late Binding & Dynamic Dispatch)|Despacho dinâmico (*late binding*)]]:** A decisão de qual instrução de máquina executar é postergada para o momento em que o código está rodando no computador, garantindo modularidade extrema.
