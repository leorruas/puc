...
# Namespaces e partial classes (espaços de nomes e modularização em larga escala)
...
## Namespaces, escopo e colisão de nomes
...
**Namespaces como mecanismo de organização e resolução de nomes.**
...
**Classes homônimas em namespaces diferentes.**
...
**Namespace e DLL são conceitos diferentes.**

É muito comum confundir esses dois conceitos fundamentais de modularização (veja [[Glossário de conceitos#Biblioteca de vínculo dinâmico Dynamic Link Library DLL Assembly|DLL no glossário]]):
...
## Arquivos, pastas e namespaces na organização do projeto
...
### Uma classe por arquivo como convenção de organização
...
**Agrupamento por camada técnica.**
...
## `using`, aliases e resolução de ambiguidades
...
### Importação de namespaces com `using`
...
### Aliases para tipos com nomes ambíguos
...
## Classes parciais e divisão da implementação

Imagine que você e um colega de equipe precisam **escrever um livro juntos**:

* **A regra padrão da POO tradicional:** Seria como se houvesse apenas um único caderno de papel físico na mesa. Enquanto seu colega está escrevendo o capítulo de validações no caderno, você fica de braços cruzados esperando ele terminar para não rabiscarem a mesma folha (**bloqueios e conflitos de edição**).
* **Com Classes Parciais (*Partial Classes*):** Cada autor recebe uma folha solta separada (`.cs`). Você escreve os atributos de estado na sua folha (`Mercadoria.Estrutura.cs`), seu colega escreve as operações de cálculo na folha dele (`Mercadoria.Operacoes.cs`), e o robô gerador de telas escreve o visual na folha dele (`Mercadoria.Designer.cs`).
* Na hora de publicar o livro (a compilação), o **compilador junta todas as folhas em um único volume encadernado e indivisível**!

---

### Uma classe distribuída em vários arquivos

1. **Permitir que uma única classe viva em múltiplos arquivos físicos `.cs`:**
   - Quebra a limitação rígida de *"uma classe = um único arquivo"*, dando flexibilidade para sistemas corporativos de grande porte.
2. **Impedir a perda de código em ferramentas automáticas (*Code Generators*):**
   - Designers visuais (como Windows Forms, WPF e Entity Framework) geram centenas de linhas de código estrutural. Com `partial class`, o código gerado pela máquina fica em um arquivo e o código escrito pelo ser humano fica em outro, evitando que uma recompilação acidental apague a lógica de negócio do programador.
3. **Redução drástica de conflitos no Git (*Merge Conflicts*):**
   - Múltiplos engenheiros de software podem trabalhar na mesma entidade ao mesmo tempo sem alterar as mesmas linhas do mesmo arquivo físico.
...
flowchart TD
    subgraph Particionamento ["Particionamento físico de uma classe parcial"]
        ...
        Compilador["Compilador Roslyn (Roslyn Compiler)"]
...
### Combinação das partes pelo compilador
Durante o processo de compilação (*build time*), o **compilador Roslyn** (*.NET Compiler Platform* — veja [[Glossário de conceitos#Compilador Roslyn NET Compiler Platform|Compilador Roslyn no glossário]]) analisa as Árvores de Sintaxe Abstrata (AST) de todos os arquivos do projeto simultaneamente.

Quando encontra declarações com o modificador `partial` que compartilham o mesmo nome e namespace, o Roslyn **funde todas as partes em uma única definição consolidada de tipo antes de gerar a Linguagem Intermediária (IL) no binário**. Na memória *Heap* do sistema em tempo de execução, a classe existe como um único bloco indivisível.

---

## Geração de código, colaboração e organização com `partial`

As classes parciais foram introduzidas para resolver desafios concretos da engenharia de software em larga escala:

### Código gerado e código escrito manualmente
* Ferramentas de interface gráfica (WPF, WinForms), mapeadores de banco de dados como o **[[Glossário de conceitos#Mapeador objeto relacional e Entity Framework ORM Entity Framework|Entity Framework (ORM)]]** e geradores de código (*Source Generators*) geram centenas de linhas de código estrutural burocrático (como mapeamento de colunas de tabelas ou `InitializeComponent()`).
* Se você editar um arquivo gerado automaticamente, a ferramenta **sobrescreverá suas alterações** na próxima compilação.
* Com `partial class`, a ferramenta grava o código em `Cliente.Database.cs`, enquanto você escreve suas regras de negócio e validações manuais com segurança em `Cliente.cs`.

### Divisão de trabalho entre desenvolvedores
...
### Separação de responsabilidades de suporte
...
## Regras de compilação para classes parciais
...
## Namespaces e classes parciais em um exemplo integrado
...
## Boas práticas para namespaces e classes parciais
...
## Conexão interdisciplinar: namespaces e o Diagrama de Pacotes da UML

A estrutura de namespaces no código é a materialização direta do **Diagrama de Pacotes** estudado na disciplina de [[07. Engenharia de Requisitos/10. Modelagem estrutural e modularização com diagrama de pacotes da uml.md|Engenharia de Requisitos]]:

* **Correspondência Direta:** Cada "pasta com aba" modelada na UML (ex.: `Empresa.Financeiro` e `Empresa.Estoque`) corresponde a um bloco `namespace Empresa.Financeiro` no código C#.
* **Importação e Dependência:** Quando um pacote aponta uma dependência com a seta `«use»` para outro pacote na UML, isso se traduz na diretiva `using OutroNamespace;` no topo do arquivo `.cs`.
* **Organização Arquitetural Limpa:** Assim como o Diagrama de Pacotes organiza dezenas de classes em agrupadores lógicos para evitar "diagramas em teia de aranha", os namespaces organizam centenas de arquivos de código em módulos coesos e desacoplados.
...
* **Próximo artigo:** [[15. Herança (generalização, especialização e extensibilidade modular)]]
