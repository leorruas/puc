...
3. **Mapeamento 1-para-1 entre Pastas e Namespaces:**
   - Embora o compilador do C# permita declarar qualquer namespace em qualquer pasta, a convenção padrão da Microsoft estabelece que o caminho da pasta deve ditar o namespace:
     - Pasta física: `MeuApp/Servicos/Pagamentos/` $\rightarrow$ Namespace lógico: `MeuApp.Servicos.Pagamentos`.
4. **Quando abrir exceção para mais de uma classe no mesmo arquivo?**
   - Apenas para **classes aninhadas privadas (*Private Nested Classes*)** ou pequenas estruturas auxiliares internas que só fazem sentido e só são visíveis dentro daquela classe principal.

---

## 4. Diretiva `using`, apelidos (*Aliases*) e resolução de ambiguidades

Para consumir classes declaradas em outros namespaces sem precisar digitar o caminho hierárquico completo (*Fully Qualified Name*), usamos a diretiva **`using`**:

```csharp
using Banco.Dominio.Contas;
using Banco.Seguranca;

// Uso direto sem prefixo:
var conta = new ContaCorrente();
```

### Resolução de conflito com Apelidos (*Type Aliasing*):
Quando importamos dois namespaces que contêm uma classe com o mesmo nome exato (ex.: `System.Drawing.Image` e `Microsoft.UI.Xaml.Controls.Image`), o compilador gera um erro de ambiguidade. Resolvemos criando um **alias explícito**:

```csharp
using System;
using ImagemGDI = System.Drawing.Image;
using ImagemUI = Microsoft.UI.Xaml.Controls.Image;

public class Renderizador 
{
    public void Processar() 
    {
        ImagemGDI fotoFisica = null; // Refere-se a System.Drawing.Image
        ImagemUI controleTela = null; // Refere-se ao controle XAML
    }
}
```
