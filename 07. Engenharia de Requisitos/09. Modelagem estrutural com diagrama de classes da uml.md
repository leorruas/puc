...
## Diagrama de classes como representação estrutural do domínio
...
## Classes, atributos, operações e multiplicidades
...
* **Simbologia de visibilidade:**
  * `+` **Público (*public*):** Pode ser acessado por outras classes (o membro é visível e acessível livremente por qualquer classe do sistema).
  * `-` **Privado (*private*):** Só a própria classe acessa (o membro é escondido externamente, garantindo a integridade interna e a regra de ouro do encapsulamento).
  * `#` **Protegido (*protected*):** A própria classe e suas subclasses podem acessar (visível na própria classe e em suas subclasses herdeiras, permitindo reaproveitamento estrutural em hierarquias de herança).
  * `~` **Pacote/interno (*package / internal*):** Acesso restrito ao mesmo pacote ou módulo, embora apareça menos nesses trabalhos acadêmicos e projetos convencionais.
...
#### Encapsulamento e escolha de visibilidade
...
#### Operações e assinaturas de métodos
...
### Relacionamentos estruturais em conjunto
...
### Agregação como relação todo-parte fraca
...
### Composição como relação todo-parte forte
...
### Herança como generalização e especialização
...
#### Exemplo de herança em código e UML
```
...
### Herança não possui multiplicidade
...
#### Por que multiplicidade não se aplica à herança
...
## Identificando relacionamentos a partir de requisitos
...
### Classe abstrata, herança e interface a partir dos requisitos

Na transição dos [[07. Engenharia de Requisitos/04. Requisitos funcionais e não-funcionais.md|requisitos funcionais (RF)]] para a modelagem estrutural de classes, o analista de requisitos identifica pistas gramaticais e regras de negócio no texto da especificação para decidir a estrutura arquitetural exata:

#### Quando há generalização e especialização
* **Pistas textuais no requisito funcional:**
  * O texto descreve subtipos, variações ou categorias que compartilham características em comum.
  * Expressões típicas: *"O sistema deve suportar diferentes tipos de X (como A e B)"*, *"Um X pode ser do tipo Y ou Z"*, *"Tanto administradores quanto clientes devem poder..."*.
* **O teste mental "É UM" (*Is-a*):**
  * `ContaCorrente` *é uma* `ContaBancaria`? Sim.
  * `Professor` *é uma* `Pessoa`? Sim.
* **Exemplo de requisito funcional:**
  * *"RF01: O sistema deve permitir o cadastro de funcionários, sendo que funcionários horistas recebem por hora trabalhada e funcionários mensalistas recebem salário fixo, compartilhando matrícula, nome e CPF."*
  * **Decisão na modelagem:** `Funcionario` será a superclasse (generalização) e `FuncionarioHorista` e `FuncionarioMensalista` serão subclasses derivadas (especializações) com a seta triangular fechada branca (`<|--`).

#### Quando a superclasse deve ser abstrata
* **Pistas textuais no requisito funcional:**
  * O conceito pai é puramente uma categoria conceitual e **nunca existe solto sozinho na vida real** dentro do negócio.
  * Expressões típicas: *"Não é permitido cadastrar um registro genérico sem especificar seu tipo"*, *"Todo pagamento deve ser obrigatoriamente por cartão ou boleto"*.
* **O teste de instanciação no negócio:**
  * Na vida real, o cliente entra no banco e diz: *"Quero abrir uma Conta genérica, que não é nem corrente nem poupança"*? Não. Toda conta física instanciada na memória precisa ser de um tipo concreto específico.
* **Decisão na modelagem:**
  * A superclasse recebe a notação em ***itálico*** ou `{abstract}`.
  * Seus métodos comuns com regras que mudam em cada subtipo (como `calcularSalario()` ou `sacar()`) são marcados como métodos abstratos, forçando as subclasses concretas a implementá-los.

#### Quando modelar uma interface
* **Pistas textuais no requisito funcional:**
  * O requisito define uma **capacidade, papel transversal ou contrato de comportamento** compartilhado por classes que **não pertencem à mesma família biológica/hierárquica**.
  * Expressões típicas: Adjetivos terminados em "-ável" ou papéis operacionais: *"deve ser imprimível"*, *"deve ser autenticável"*, *"deve poder ser exportado para PDF"*, *"todo item tributável deve calcular seu imposto"*.
* **O teste de família vs. habilidade:**
  * `Carro` e `RelatorioMensal` pertencem à mesma árvore genealógica de herança? Não. Mas ambos podem precisar implementar a interface `«Imprimivel»`.
  * `Usuario` e `SensorIoT` não são parentes, mas ambos podem implementar `«Autenticavel»`.
* **Decisão na modelagem:**
  * Modela-se a caixa com o estereótipo `«interface»` contendo apenas as assinaturas das operações.
  * As classes concretas conectam-se a ela pela relação de **realização** (linha pontilhada com seta triangular branca `..|>`).

#### Decisão prática a partir dos requisitos

| O que você lê no requisito funcional? | O que isso representa? | Elemento correspondente no diagrama UML |
| :--- | :--- | :--- |
| *"Existem tipos específicos de X que compartilham atributos e regras"* | Família de dados comuns com especialização | **Herança** (seta triangular fechada contínua `<|--`) |
| *"O elemento genérico X não pode ser criado diretamente sem seu subtipo"* | Conceito apenas conceitual / gabarito base | **Classe abstrata** (nome em *itálico* ou `{abstract}`) |
| *"Objetos de diferentes áreas devem oferecer o mesmo comportamento/habilidade"* | Contrato puro de comportamento transversal | **Interface** (estereótipo `«interface»` e linha pontilhada `..|>`) |
| *"Um elemento X é composto por vários elementos Y que não vivem sem ele"* | Ciclo de vida coincidente / dependência existencial | **Composição** (losango preenchido preto `*--`) |
| *"Um elemento X agrupa elementos Y que continuam existindo se X sumir"* | Vínculo todo-parte independente | **Agregação** (losango vazado branco `o--`) |

---

### Do diagrama de classes à implementação em C#

A modelagem de classes da Engenharia de Requisitos não é apenas um desenho teórico; ela é a **planta arquitetural direta** utilizada pelos desenvolvedores na codificação:

1. **Visibilidade e Encapsulamento:**
   * Os símbolos `-` (privado), `+` (público) e `#` (protegido) no diagrama traduzem-se diretamente nos modificadores de acesso do C# e Java (veja [[01. Programacao Modular/11. Princípio da ocultação da informação (information hiding e encapsulamento)|Ocultação da Informação]] e [[01. Programacao Modular/12. Modificadores de acesso (visibilidade e níveis de proteção no encapsulamento)|Modificadores de Acesso]]).
2. **Abstrações, Contratos e Polimorfismo:**
   * Classes abstratas em itálico (`{abstract}`) e interfaces (`«interface»`) desenhadas na UML definem os contratos puros que garantem o polimorfismo puro e a substituição sem efeitos colaterais no código (veja [[01. Programacao Modular/18. Classes abstratas e métodos abstratos (contratos de herança e polimorfismo puro)|Classes Abstratas]] e [[00. Sintaxe Multilinguagem/05. Abstração e encapsulamento com classes (TADs)|Abstração com Classes]]).
3. **Composição vs. Herança e o Princípio DIP (SOLID):**
   * O debate clássico da Engenharia de Software (*"Prefira composição a herança"*) reflete-se na escolha entre desenhar losangos de composição ou setas triangulares de herança. A composição viabiliza a **Inversão de Dependência (DIP)** e o desacoplamento arquitetural (veja [[01. Programacao Modular/23. Princípios SOLID de design orientado a objetos|Princípios SOLID de Design Orientado a Objetos]]).
4. **Aplicação no Projeto Integrador (Eixo 2):**
   * O Diagrama de Classes estruturado na disciplina de Engenharia de Requisitos é o entregável formal obrigatório da **Etapa 02** do projeto prático (veja [[06. Projeto - Aplicacao Interativa/03. Cronograma semanal e entregas de etapas.md|Cronograma Semanal do Projeto]]), servindo de base para a criação dos Models e Controllers do ASP.NET Core MVC.

---

## Estrutura do domínio como ponte entre requisitos e código
