...
flowchart TD
    ...
    subgraph Totem["Fronteira do sistema: Totem de autoatendimento"]
        ...
        UC3["Processar pagamento via cartão"]
        UC4["Emitir comprovante de compra"]
    end
...
```mermaid
flowchart TD
    NodeRoot["Relacionamentos em diagramas de casos de uso"] --> Node1["1. Associação<br>(Conecta ator a caso de uso)"]
    NodeRoot --> Node2["2. Generalização / Herança<br>(Conecta ator a ator ou CU a CU)"]
...
```mermaid
flowchart LR
    Cliente["Cliente (Ator)"] --> UC1["Consultar saldo (CU)"]
    UC2["Processar pagamento (CU)"] --> Gateway["Gateway de pagamento (Ator)"]
```
...
```mermaid
flowchart TD
    VIP["Cliente VIP (Ator filho)"] -->|Herança| Cliente["Cliente (Ator pai)"]
```
...
```mermaid
flowchart TD
    Pix["Pagar via Pix"] -->|Herança| Pai["Realizar pagamento"]
    Cartao["Pagar via cartão"] -->|Herança| Pai
```
...
```mermaid
flowchart LR
    Transf["Realizar transferência"] -.->|«include»| Validar["Validar token de segurança"]
...
```mermaid
flowchart LR
    Cupom["Aplicar cupom de desconto"] -.->|«extend»| Compra["Efetuar compra"]
```
...
```mermaid
flowchart LR
    Saldo["Notificar saldo insuficiente"] -.->|«extend»| Saque["Realizar saque"]
```
