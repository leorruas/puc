...
A organização abaixo separa essas duas dimensões e acrescenta ressalvas técnicas quando uma simplificação poderia gerar um modelo mental errado.
...
Componentes do servidor também podem ser descritos por uma **topologia**. Topologia descreve como nós, serviços ou componentes estão dispostos e conectados. Já um **tier** representa uma separação física ou de implantação.
...
Uma analogia útil é pensar em peças de Lego: a estrutura básica já está disponível e os componentes podem ser substituídos, atualizados ou preenchidos conforme a navegação.
...
O ponto mais importante é separar estrutura de navegação de estratégia de renderização.
...
- **Blazor é um framework da Microsoft para construção de interfaces Web com .NET.**
