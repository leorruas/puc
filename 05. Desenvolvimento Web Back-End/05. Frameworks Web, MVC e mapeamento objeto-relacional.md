...
## Exemplo completo consolidado: gênero e filme com EF Core

A transcrição do vídeo mostra um exemplo completo de ponta a ponta: criar as entidades `Genero` e `Filme`, representar o relacionamento 1:N, configurar um `DbContext`, gerar uma migration, criar o banco e inserir registros.

Como a transcrição automática possui vários trechos corrompidos, o código abaixo é uma **reconstrução didática do fluxo demonstrado**, e não uma reprodução literal do código exibido pelo professor.
...
public class ApplicationContext : DbContext
{
    ...
    {
        // Configuração semelhante ao fluxo demonstrado
        // pelo professor com SQL Server Express no Windows.
...
No fluxo mostrado pelo professor no Visual Studio/Windows, os comandos são executados no **Package Manager Console**:
...
O vídeo utiliza o fluxo do Visual Studio no Windows e menciona o pacote de ferramentas do Entity Framework. No macOS, a combinação `Microsoft.EntityFrameworkCore.Design` + `dotnet-ef` é a forma equivalente para executar migrations pela linha de comando.
