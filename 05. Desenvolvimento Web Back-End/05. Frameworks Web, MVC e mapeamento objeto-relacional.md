...
> **Contexto:** Este artigo conecta frameworks Web, MVC, Razor, roteamento e scaffolding ao mapeamento objeto-relacional com Entity Framework Core. O objetivo é mostrar como estrutura da aplicação e persistência se integram, incluindo relacionamentos, `DbContext`, `DbSet<T>`, migrations e configuração em diferentes ambientes.
...
O código abaixo consolida o fluxo completo em uma versão didática: entidades, relacionamento 1:N, contexto, configuração e persistência.
...
**Próximo artigo:** [[05. Desenvolvimento Web Back-End/06. Model, validação e persistência no ASP.NET Core MVC|Model, validação e persistência no ASP.NET Core MVC]]
