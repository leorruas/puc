...
## Frameworks Web: estrutura reutilizável
...
## MVC: organização de responsabilidades
...
```mermaid
flowchart LR
    Usuario["Usuário / navegador"]:::external
    Rota["Roteamento"]:::component
    Controller["Controller"]:::core
    Model["Model / serviços"]:::component
    View["View"]:::component

    Usuario -->|"requisição"| Rota
    Rota --> Controller
    Controller --> Model
    Model --> Controller
    Controller --> View
...
## ORM: ponte entre objetos e banco relacional
...
```mermaid
flowchart LR
    Genero["Objeto Genero"]:::component
    Filmes["Coleção de Filme"]:::component
    ORM["ORM<br>mapeamento"]:::core
    TGenero[("Tabela Generos")]:::data
    TFilmes[("Tabela Filmes<br>GeneroId FK")]:::data
...
## Entity Framework Core: ORM no ecossistema .NET
...
Se o atributo for usado, também é válido passar o nome diretamente como texto:
...
As duas formas informam a mesma propriedade ao `ForeignKeyAttribute`. A diferença é que `nameof(GeneroId)` é normalmente preferível porque o compilador verifica se `GeneroId` existe e acompanha renomeações feitas pelas ferramentas de refatoração. Já `"GeneroId"` é apenas uma string: se a propriedade for renomeada e o texto não for atualizado, o compilador não detecta esse erro.
...
Essa distinção conecta diretamente o conteúdo com [[02. Modelagem de Dados/11. Mapeamento de relacionamentos#Relacionamentos binários: 1:1, 1:N e N:N|Mapeamento de relacionamento 1:N]].
...
```mermaid
flowchart LR
    Classes["Classes C#<br>Genero e Filme"]:::component
    Modelo["Modelo do EF Core"]:::core
    Contexto["DbContext"]:::component
    Operacoes["Consultas e alterações"]:::component
    SQL["SQL gerado pelo provider"]:::data
    Banco[("Banco relacional")]:::data
    Migration["Migrations"]:::external
    ...
    SQL --> Banco
    Modelo --> Migration
    Migration -->|"evolui esquema"| Banco
```
...
## Exemplo completo: gênero e filme com EF Core
...
```mermaid
flowchart LR
    Codigo["Código C#"]:::external
    Genero["Objeto Genero"]:::component
    EF["Entity Framework Core"]:::core
    Banco[("Banco de dados")]:::data
    Filme["Objeto Filme<br>com GeneroId"]:::component

    Codigo -->|"new Genero"| Genero
    Genero -->|"Add + SaveChanges"| EF
    EF -->|"INSERT"| Banco
    Banco -->|"Id gerado"| Genero
    Genero --> Filme
    Filme -->|"Add + SaveChanges"| EF
```
...
### Separação entre objetos, mapeamento e banco
...
## Framework, MVC e ORM no mesmo fluxo
...
## Tutorial: configurar EF Core no Visual Studio Code no macOS
...
### Instalar o suporte a C# no Visual Studio Code
...
### Configurar o `DbContext` para SQLite
...
### Se o projeto precisar usar SQL Server
