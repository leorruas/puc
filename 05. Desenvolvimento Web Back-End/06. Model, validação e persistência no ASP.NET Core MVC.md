...
## Da requisição HTTP à persistência
...
## Data Annotations: metadados e validação
...
## `ApplicationDbContext`: especializando o `DbContext` para a aplicação
...
## Injeção de dependência: como o contexto chega ao Controller
...
## Controller: da requisição HTTP à resposta
...
### Controller e action não são a mesma coisa
...
### Como a URL encontra Controller e action
...
```mermaid
flowchart LR
    URL["GET /Pessoas/Details/5"]:::external
    Rota["Roteamento"]:::component
    Controller["PessoasController"]:::core
    Action["Details(5)"]:::component
...
### Actions GET e POST representam momentos diferentes do fluxo
...
### Model binding: como dados HTTP viram parâmetros C#
...
```mermaid
flowchart LR
    Form["Formulário<br>Nome + Email"]:::external
    Post["POST"]:::component
    Binding["Model binding"]:::core
...
### `ModelState`: onde a validação chega ao Controller
...
```mermaid
flowchart LR
    Dados["Dados do POST"]:::external
    Binding["Model binding"]:::component
    Model["Pessoa"]:::data
    Validacao["Data Annotations<br>e validação"]:::component
    State["ModelState"]:::core
    Persistir["Persistir"]:::data
    View["Retornar View"]:::warning

    Dados --> Binding
    Binding --> Model
    Model --> Validacao
    Validacao --> State
    State -->|"válido"| Persistir
    State -->|"inválido"| View
```
...
### Actions de consulta: `Index` e `Details`
...
### O Controller escolhe resultados que viram respostas HTTP
...
#### Limitação de campos e overposting
...
### Scaffolding mostra essas convenções funcionando juntas
...
**Onde entram as migrations neste fluxo?**
...
```mermaid
flowchart TD
    Navegador["Navegador"]:::external
    Rota["Roteamento"]:::component
    Action["Controller / action"]:::core
    Binding["Model binding"]:::component
    State["ModelState"]:::component
    Contexto["ApplicationDbContext"]:::data
    EF["EF Core / provider"]:::data
    Banco[("Banco")]:::data
    View["View"]:::component
    Resposta["Redirecionamento<br>ou status HTTP"]:::external

    Navegador --> Rota
    Rota --> Action
    Action --> Binding
    Binding --> State
    State -->|"válido em escrita"| Contexto
    Contexto --> EF
    EF <--> Banco
    EF --> Action
    State -->|"inválido"| View
...
**`Startup.cs` e `Program.cs`: o que mudou?**
