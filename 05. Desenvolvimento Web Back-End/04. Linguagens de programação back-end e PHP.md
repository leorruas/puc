...
A ideia central desta aula é mais ampla do que PHP. Uma aplicação back-end precisa de algum programa executando no servidor que consiga receber uma requisição, interpretar o que foi solicitado, processar dados e produzir uma resposta. A própria PUC destaca que **não existe uma única linguagem obrigatória para fazer back-end**: diferentes linguagens podem cumprir esse papel, desde que o ambiente de execução ofereça os recursos necessários para responder às requisições Web.

O conteúdo da aula usa **PHP como exemplo principal**, mas também apresenta critérios gerais para comparar linguagens e ambientes back-end: forma de execução, sistema de tipos, portabilidade, integração com servidor Web, acesso a banco de dados e mecanismos de template. C# aparece nas anotações como contraste de tipagem e o ASP.NET Core MVC é citado como conteúdo posterior da disciplina.
...
A aula introduz alguns eixos que ajudam a entender diferenças entre linguagens e ambientes de execução.

| Característica | Pergunta principal | Exemplos que aparecem na aula |
| --- | --- | --- |
| **Modelo de execução** | O código é compilado, interpretado ou usa uma estratégia híbrida? | PHP, JavaScript e C# aparecem nas anotações como contraste |
...
| **Acesso a dados** | Como a aplicação se conecta a bancos e outras fontes? | A aula cita integração com bancos de dados |
| **Templates** | Como HTML e conteúdo dinâmico são combinados? | PHP e, posteriormente, Razor no ASP.NET Core MVC |

Portanto, o plural “linguagens de programação back-end” no título da aula faz sentido porque os conceitos são gerais. O **PHP é o estudo de caso concreto desta sessão**, não a definição inteira de back-end.
