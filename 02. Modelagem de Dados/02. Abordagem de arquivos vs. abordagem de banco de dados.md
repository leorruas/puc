...
```mermaid
flowchart TD
    Vendas["Vendas"]:::external
    Producao["Produção"]:::external
    Compras["Compras"]:::external
    ArqVendas[("arquivo de vendas")]:::data
    ArqProducao[("arquivo de produção")]:::data
    ArqCompras[("arquivo de compras")]:::data

    Vendas --> ArqVendas
    Producao --> ArqProducao
    Compras --> ArqCompras
```
...
flowchart TD
    ...
    Compras["Compras"]:::external
    SGBD["SGBD"]:::core
    Dados[("Base integrada de dados")]:::data
...
```mermaid
flowchart LR
    Usuarios["Usuários e aplicações"]:::external
    SGBD["SGBD<br>controle, consulta e manipulação"]:::core
    Banco[("Banco de dados<br>dados persistentes + metadados")]:::data

    Usuarios -->|"instruções e consultas"| SGBD
    SGBD -->|"gerencia"| Banco
```
...
```mermaid
flowchart LR
    Realidade["Mundo real"]:::external
    MiniMundo["Mini-mundo<br>fatos relevantes ao domínio"]:::core
    Modelo["Modelo de dados<br>estrutura e relacionamentos"]:::data

    Realidade -->|"seleção e abstração"| MiniMundo
    MiniMundo -->|"modelagem"| Modelo
```
