...
```mermaid
flowchart TD
    subgraph SubgraphAplicacoes["1. Clientes e Aplicações"]
        Apps["Usuários e Aplicações<br>Web, Mobile, Desktop, APIs"]
    end

    subgraph SubgraphSGBD["2. Camada do SGBD (Software Gerenciador)"]
        Motor["Motor de Execução e Otimizador SQL<br>Controle de transações ACID e concorrência<br>Segurança, permissões e buffers"]
    end

    subgraph SubgraphBD["3. Camada do Banco de Dados (Repositório)"]
        Dados[("Banco de Dados Integrado<br>Tabelas, registros, relacionamentos<br>Catálogo e dicionário de dados")]
    end

    Apps -->|"Comandos declarativos SQL"| Motor
    Motor -->|"Leitura e escrita física estruturada"| Dados
```
...
| Vantagem central | Significado prático no desenvolvimento de software |
| :--- | :--- |
| **Autocontenção e metadados** | O banco guarda sua própria estrutura no catálogo (dicionário de dados), permitindo que aplicações interajam sem conhecer offsets de memória. |
| **Múltiplas visões dos dados** | Diferentes usuários enxergam apenas os dados relevantes para seu perfil de acesso através de visões (*views*). |
| **Transações confiáveis (ACID)** | Garantia de que operações financeiras ou transações críticas ocorram de forma atômica (ou tudo é executado ou tudo é desfeito). |
| **Padrão declarativo universal** | Uso da linguagem SQL padronizada internacionalmente pela ISO/ANSI para manipulação e definição de esquemas. |
...
flowchart TD
    subgraph SubgraphTradicional["1. Abordagem Tradicional em Arquivos"]
        App1["Aplicação de Vendas<br>Lógica de negócio e formato do arquivo"]
        File1[("Arquivo de Vendas<br>clientes_vendas.dat")]
        App2["Aplicação Financeira<br>Lógica de negócio e formato do arquivo"]
    ...
    subgraph SubgraphBancoDados["2. Abordagem de Banco de Dados"]
        ...
        BD[("Banco de Dados Único<br>Dados integrados e catálogo")]
