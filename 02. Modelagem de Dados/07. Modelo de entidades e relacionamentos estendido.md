...
```mermaid
flowchart TD
    Banco["BANCO<br>entidade forte"]:::core
    Codigo(["<u>Codigo</u>"]):::data
    Nome(["Nome"]):::data
    Endereco(["Endereco"]):::data
    Possui{"POSSUI<br>relacionamento identificador"}:::component
    Agencia["AGENCIA<br>entidade fraca"]:::warning
    EndAgencia(["Endereco"]):::data
    Numero(["<u>Num_agencia</u><br>chave parcial"]):::data

    Banco --- Codigo
    Banco --- Nome
    Banco --- Endereco
    Banco ---|"1 (1,1)"| Possui
    Possui ---|"N (1,N) total"| Agencia
    Agencia --- EndAgencia
    Agencia --- Numero
```
...
flowchart TD
    ...
    Matricula(["<u>Matricula</u>"]):::component
    Nome(["Nome"]):::component
    Atua{"ATUA"}:::core
    Projeto["PROJETO"]:::data
    Funcao(["Função<br>atributo do relacionamento"]):::component
    Codigo(["<u>Codigo</u>"]):::component
    Titulo(["Título"]):::component

    Funcionario --- Matricula
    Funcionario --- Nome
    Funcionario ---|"(1,N)"| Atua
    Atua ---|"(0,N)"| Projeto
    Atua --- Funcao
    Projeto --- Codigo
    Projeto --- Titulo
...
flowchart TD
    ...
    Trabalha{"TRABALHA_EM"}:::core
    DataInicio(["Data_Inicio"]):::component

    Funcionario ---|"N"| Trabalha
    Trabalha ---|"1"| Departamento
    Trabalha ---|"1"| Localidade
    Trabalha --- DataInicio
```
...
```mermaid
flowchart TD
    Q{"A associação só existe<br>com as três entidades juntas?"}:::core
    Ternario["Use relacionamento ternário<br>quando a associação é indivisível"]:::component
    Binarios["Use relacionamentos binários<br>quando as associações são independentes"]:::component
