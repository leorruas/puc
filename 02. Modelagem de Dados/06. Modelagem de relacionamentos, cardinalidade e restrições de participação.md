...
Para visualizar com clareza as associações e como as anotações de cardinalidade ficam posicionadas diretamente sobre as linhas de conexão:
...
flowchart TD
    ...
    subgraph SubgraphTotalidade["3. Totalidade (Dependência de Existência) — Sócio e Dependente"]
        Socio["SÓCIO"] ---|"(0, N) — Parcial (Linha Simples)"| RelDep{"POSSUI"}
        RelDep ===|"(1, 1) — Total (Linha Dupla)"| Dep["DEPENDENTE"]
    end

    Subgraph1Para1 --> Subgraph1ParaN --> SubgraphTotalidade
```
...
flowchart TD
    subgraph SubgraphPasso1["Passo 1: Fixar o lado esquerdo e olhar para o direito"]
        P1["Dado UM Empregado, a quantos Departamentos ele pertence?<br>Mínimo: 1 | Máximo: 1 -> Anotação na linha: (1, 1)"]
    end

    subgraph SubgraphPasso2["Passo 2: Fixar o lado direito e olhar para o esquerdo"]
        P2["Dado UM Departamento, quantos Empregados trabalham nele?<br>Mínimo: 1 | Máximo: N -> Anotação na linha: (1, N)"]
    end
...
```mermaid
flowchart TD
    subgraph Subgraph11["1. Relacionamento Um para Um (1:1)"]
        T1["TÉCNICO"] ---|"1 (1, 1)"| R1{"COMANDA"} ---|"1 (1, 1)"| TM1["TIME"]
    end

    subgraph Subgraph1N["2. Relacionamento Um para Muitos (1:N)"]
        P1["PROPRIETÁRIO"] ---|"1 (0, N)"| R2{"POSSUI"} ---|"N (1, 1)"| V1["VEÍCULO"]
    end

    subgraph SubgraphNM["3. Relacionamento Muitos para Muitos (N:M)"]
        M1["MÉDICO"] ---|"N (0, N)"| R3{"ATENDE"} ---|"M (0, M)"| PC1["PACIENTE"]
    end

    Subgraph11 --> Subgraph1N --> SubgraphNM
```

| Tipo de cardinalidade | Notação sobre a linha | Descrição formal | Exemplo de negócio |
| :--- | :--- | :--- | :--- |
| **Um para Um ($1:1$)** | `1` $\longleftrightarrow$ `1` | Cada ocorrência de A associa-se a no máximo uma de B, e vice-versa. | `Técnico` *comanda* `Time`. |
| **Um para Muitos ($1:N$)** | `1` $\longleftrightarrow$ `N` | Uma ocorrência de A associa-se a várias de B, mas cada B liga-se a apenas um A. | `Proprietário` *possui* `Veículos`. |
| **Muitos para Um ($N:1$)** | `N` $\longleftrightarrow$ `1` | É a visão inversa do relacionamento $1:N$. | Vários `Veículos` *pertencem a* um único `Proprietário`. |
| **Muitos para Muitos ($N:M$)** | `N` $\longleftrightarrow$ `M` | Múltiplas ocorrências de A associam-se livremente a múltiplas de B. | `Médicos` *atendem* `Pacientes`. |
...
flowchart TD
    ...
    subgraph SubgraphBinario["2. Grau Binário"]
        Prop2["PROPRIETÁRIO"] ---|"1"| RelProp2{"POSSUI"} ---|"N"| Veic2["VEÍCULO"]
    end
    ...
    SubgraphUnario --> SubgraphBinario --> SubgraphTernario
```
