...
> **Contexto:** Estudo detalhado dos componentes de cabeçalho em C# e Java (`using System`, `class Program`, `void`, `return`, `static void Main`), com foco no papel do sublinhado (`_`) para atributos privados, do modificador `static`, da visibilidade (`public` vs `private`) e o uso da Técnica de Feynman.
...
## 2. Por que usar o sublinhado (`_`) no início de atributos (`_saldo`, `_criacao`)?
...
## 3. A palavra-chave `static`: Membro da Classe vs. Membro da Instância
...
## 4. Quando usar `static` e quando NÃO usar?
...
## 5. Combinações de visibilidade e escopo
...
## 6. Comparativo Multilinguagem (`static`)
