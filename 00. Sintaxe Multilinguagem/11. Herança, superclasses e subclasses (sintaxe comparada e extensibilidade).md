...
# Herança, superclasses e subclasses (sintaxe comparada e extensibilidade)
...
## Glossário de terminologias equivalentes em hierarquias de classes
...
## Herança especializa um tipo e encadeia inicialização ancestral
...
## Comparação de sintaxe de herança
...
## Fechar uma hierarquia não é o mesmo que tornar dados imutáveis
...
| Conceito | C# | Java | Python | JavaScript / TypeScript |
| :--- | :--- | :--- | :--- | :--- |
| Restringir reatribuição / mutação de referência | `const`, `readonly` conforme o caso | `final` | convenções, propriedades e tipos imutáveis conforme o caso | `const` impede reatribuição da variável, não torna o objeto imutável |
| Impedir extensão de classe | `sealed class` | `final class` | `@final` atua principalmente para verificadores de tipo, não como bloqueio de runtime | não há equivalente direto universal em JavaScript; TypeScript pode restringir por desenho/API |
| Impedir nova sobrescrita | `sealed override` | `final method` | `@final` pode sinalizar a restrição para type checkers | depende do padrão de projeto e das restrições da linguagem/ferramenta |

### Restrição de reatribuição e restrição de herança são conceitos diferentes

* **Para variáveis e valores (`const` / `readonly` / `final`):**
  - Impede que o **conteúdo de uma variável ou campo** seja alterado após sua inicialização (veja [[00. Sintaxe Multilinguagem/01. Variáveis, tipos e atribuição#Constantes e imutabilidade (const, final, readonly)|Guia de Sintaxe 01 (Constantes)]]).
...
* **Glossário da disciplina:** [[01. Programacao Modular/Glossário de conceitos#Classe selada (Sealed Class / Final Class)|Glossário 62 (Classe selada)]] e [[01. Programacao Modular/Glossário de conceitos#Membro selado (Sealed Member / Sealed Override)|Glossário 63 (Membro selado)]]
