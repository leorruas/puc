...
# Tipos genéricos e polimorfismo paramétrico (Generics, TypeVar, Any, Templates)

> **Contexto:** Estudo comparativo e transversal de tipos genéricos e polimorfismo paramétrico em C#, Java, Python e TypeScript/JavaScript. O artigo compara sintaxe, restrições e, principalmente, o que cada ecossistema preserva ou apaga quando o programa chega ao runtime.

---

## Generics parametrizam tipos sem duplicar a lógica
...
| Ecossistema | Declaração típica | O que acontece no runtime |
| :--- | :--- | :--- |
| C# / .NET | `class Caixa<T>` | O CLR preserva informação sobre tipos genéricos. O JIT normalmente gera código especializado para tipos de valor e pode compartilhar implementações para tipos de referência. |
| Java / JVM | `class Caixa<T>` | A maior parte dos parâmetros genéricos é apagada para o limite declarado ou `Object`; casts e métodos de ponte podem ser inseridos pelo compilador. |
| TypeScript / JavaScript | `class Caixa<T>` | A verificação ocorre antes da execução e os tipos TypeScript são removidos do JavaScript emitido. |
| Python | `Generic[T]` / `TypeVar` | As anotações podem permanecer acessíveis em runtime, mas o interpretador padrão não aplica automaticamente as restrições de tipo. |

---

## Polimorfismo paramétrico sustenta coleções e APIs reutilizáveis
...
## Comparação entre os quatro ecossistemas
...
| **Mecanismo de execução** | O CLR preserva informação genérica; o JIT especializa tipos de valor e pode compartilhar código entre tipos de referência | A JVM usa principalmente *type erasure* para os parâmetros genéricos | Anotações permanecem disponíveis como metadados, mas o runtime padrão não as impõe | Os tipos do TypeScript são apagados na emissão de JavaScript |
...
## Implementação comparada nas quatro linguagens
...
# Declaração das variáveis de tipo com nomes em maiúsculo
T = TypeVar('T')
TKey = TypeVar('TKey')
TValue = TypeVar('TValue')

# Classe genérica herdando de Generic[T]
...
## Reificação, apagamento de tipos e anotações em runtime
