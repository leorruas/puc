...
```mermaid
flowchart TD
    Topo["C — topo<br>próximo a sair"]:::core
    Meio["B"]:::component
    Base["A — base<br>primeiro a entrar"]:::data
...
```mermaid
flowchart LR
    Entrada["C — fim<br>entrada"]:::data
    Meio["B"]:::component
    Saida["A — frente<br>próximo a sair"]:::core
...
| Chave | Hash direciona para | Valor |
| :--- | :--- | :--- |
| `Livro` | posição interna calculada | `R$ 59,90` |
| `Caneta` | posição interna calculada | `R$ 3,50` |
...
```mermaid
flowchart LR
    Requisitos["Requisitos<br>necessidades e regras"]:::external
    IHC["Design de interação<br>modelo mental e experiência"]:::component
    Estrutura["Estrutura de dados<br>pilha, fila, hash, árvore, grafo"]:::core

    Requisitos -->|"define necessidades"| Estrutura
    IHC -->|"orienta comportamento percebido"| Estrutura
```
...
| Namespace / família | Exemplos | Quando usar |
| :--- | :--- | :--- |
| `System.Collections` | `ArrayList`, `Hashtable`, `Queue`, `Stack` não genéricos | APIs antigas, interoperabilidade ou casos em que a coleção não genérica seja realmente necessária; em código novo, alternativas genéricas costumam oferecer melhor segurança de tipos |
| `System.Collections.Generic` | `List<T>`, `Dictionary<TKey,TValue>`, `HashSet<T>`, `Queue<T>`, `Stack<T>` | Coleções tipadas de uso geral |
| `System.Collections.Concurrent` | `ConcurrentDictionary<TKey,TValue>`, `ConcurrentQueue<T>`, `BlockingCollection<T>` | Cenários concorrentes em que múltiplas threads acessam a coleção; a estrutura fornece operações próprias para coordenação thread-safe |
