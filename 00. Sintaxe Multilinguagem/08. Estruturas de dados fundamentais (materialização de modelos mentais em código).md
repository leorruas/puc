...
```mermaid
graph TD
    subgraph Modelos ["Modelos Mentais"]
        LIFO["LIFO (Desfazer)"]
        FIFO["FIFO (Atendimento)"]
        SEQ["Sequencial Flexível"]
        HASH["Chave-Valor Instantânea"]
        UNIQ["Unicidade sem Repetição"]
    end

    subgraph Codigo ["Materialização em C# / Java / Python / JS"]
        S["Stack<T>"]
        Q["Queue<T>"]
        L["List<T> / ArrayList"]
        D["Dictionary<TKey,TValue> / Map"]
        ST["HashSet<T> / Set"]
    end
