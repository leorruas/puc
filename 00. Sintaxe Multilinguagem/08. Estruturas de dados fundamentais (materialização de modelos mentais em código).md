...
# Estruturas de dados fundamentais (materialização de modelos mentais em código)
...
## Estruturas de dados materializam diferentes modelos de organização
...
## Pilha (*Stack* - Modelo LIFO / Desfazer)

### Organização LIFO da pilha
```mermaid
flowchart TD
    subgraph Pilha ["Pilha na memória (LIFO: acesso exclusivo no topo)"]
        E3["Elemento C (Topo)<br>Pop"]:::core
        E2["Elemento B"]:::component
        E1["Elemento A (Base)<br>Último a sair"]:::data
...
## Fila (*Queue* - Modelo FIFO / Ordem de Chegada)

### Organização FIFO da fila
```mermaid
flowchart LR
    subgraph Fila ["Fila na memória (FIFO: entrada no fim, atendimento na frente)"]
        Fim["Cliente C (Fim)<br>Enqueue"]:::data --> Meio["Cliente B"]:::component --> Frente["Cliente A (Frente)<br>Dequeue"]:::core
    end
```
...
## Lista Dinâmica (*List* - Sequência Indexada)

### Índices e ordem na lista
```mermaid
flowchart TD
...
## Tabela hash e dicionário (*Dictionary / Map* - busca por chave única)

### Chaves e valores no dicionário
```mermaid
flowchart TD
...
## Conjunto (*Set* - unicidade sem repetição)
...
### Organização hierárquica da árvore binária
```mermaid
flowchart TD
...
## Fila de prioridade (*PriorityQueue / Heap*)
...
## Grafo (*Graph* - representação por lista de adjacência)

### Vértices e arestas no grafo
```mermaid
flowchart TD
...
## Estruturas de dados respondem a necessidades de produto e interação
...
### Fila (*Queue* / FIFO) e fila de prioridade (*PriorityQueue*)
...
### Tabela hash e dicionário (*Dictionary / Map*)
...
## Coleções .NET: não genéricas, genéricas e concorrentes
...
### Por que coleções genéricas costumam ser preferidas
